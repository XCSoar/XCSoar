...
public class SAFHelper {
  ...
  public VolumeInfo[] getVolumes() {
    ...
    for (StorageVolume vol : vols) {
      ...
      if (matched != null) {
        if (isTreeUriAccessible(matched))
          ...
          Log.w(TAG, "Stale permission, releasing: " + matched);
          releaseTreePermission(Uri.parse(matched));
          matched = null;
        }
      }
    ...
    // Release persisted URIs that don't match any current volume.
    for (UriPermission p : perms) {
      String uriStr = p.getUri().toString();
      if (!matchedUris.contains(uriStr)) {
        Log.i(TAG, "Releasing orphan permission: " + uriStr);
        releaseTreePermission(p.getUri());
      }
    }
   ...
   * already exists, it is released first so that only one tree URI
   * per volume is persisted at any time.
   */
  public void persistTreePermission(Uri treeUri) {
    /* Extract the volume UUID from the new tree URI so we can check
       for an existing permission on the same volume. */
