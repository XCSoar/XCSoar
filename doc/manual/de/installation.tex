...
(Hinweis: Bei älteren Android Versionen, z.B. vor Android 7, hieß dieser Ort auch \texttt{Gerätespeicher} oder ganz früher \texttt{SD-Karte} (emuliert). Die Bezeichnung \texttt{SD-Karte} führte bei den alten Geräten oft zu Missverständnissen beim Benutzer,  denn damit war NICHT eine zusätzliche Hardware-SD-Karte gemeint, sondern nur eine ''virtuelle'' SD-Karte, die im Arbeitsspeicher des Smartphones quasi''sumuliert'' wird. Bei neueren Geräten ist dies im Dateimanager i.d.R. verständlicher bezeichnet. Der in {\textsf  XCSoar} angezeigte Pfad lautet i.d.R.  \texttt{storage/emulated/0} .)
\end{description}
...
Vom Nutzer selbst können Wegpunkt-Dateien (z.B. als \texttt{.cup} Datei), detaillierte Beschreibungen zu den Wegpunkten in einer Wegpunktdetails-Datei (\texttt{.txt}),  Luftraumdateien (\texttt{.txt}) sowie Checklisten importiert, angepasst oder auch selbst neu angelegt werden. Näheres dazu weiter unten bzw. in Kap.~\ref{cha:data-files}.
