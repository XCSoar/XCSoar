...
Die meisten modernen Android- und iOS-Geräte haben einen eingebauten GPS-Empfänger.  Viele dieser Telefone und Tablets haben auch einen barometrischen Drucksensor; {\textsf  XCSoar} verwendet ihn für die Höhe, wenn kein besseres externes Höhenmessgerät angeschlossen ist. Unter Android verwendet {\textsf  XCSoar} außerdem den Beschleunigungssensor des Telefons als G-Sensor (Lastvielfaches). Mitunter jedoch ist es ratsam, diese Daten aus einem anderen Gerät zu verwenden, wenn man z..B.\ Luftverkehrsdaten aus einem 
...
Moderne Android-Geräte haben in der Regel keine serielle Schnittstelle.
...
\subsection*{Unter iOS}\index{Installation!unter iOS}

Installiere \textsf{XCSoar} aus dem Apple App Store.  Datendateien liegen in
\texttt{Documents/XCSoarData} und können mit der Dateien-App kopiert werden.

\subsection*{Unter macOS}\index{Installation!unter macOS}

Installiere \textsf{XCSoar} aus dem Apple App Store oder lade das macOS-Paket
von der XCSoar-Homepage.  Datendateien liegen in \texttt{XCSoarData} im
Home-Verzeichnis.

\subsection*{Auf einem Windows PC}\index{Installation!unter Windows}
  
Zwei Windows-Versionen stehen zur Verfügung:
\begin{itemize}
\item \textbf{64-bit} (Ziel ``WIN64OPENGL''): die empfohlene Version.
  Installer oder \verb|.zip| für eine manuelle Installation.
\item \textbf{32-bit} (Ziel ``WIN32OPENGL''): dasselbe für 32-bit-Windows.
\end{itemize}
...
\item[macOS]
\texttt{XCSoarData} befindet sich im Home-Verzeichnis.
\item[iOS]
\texttt{XCSoarData} befindet sich im Documents-Ordner der App und ist
über die Dateien-App erreichbar.
\item[Unix/Linux]
...
Diese werden ebenfalls im \texttt{XCSoarData} Verzeichnis (Windows, macOS, iOS und Android-Geräte), oder aber im \texttt{.xcsoar} 
