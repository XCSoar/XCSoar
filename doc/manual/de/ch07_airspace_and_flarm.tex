\chapter{Lufträume, Verkehr und Teamflug}\label{cha:airspace}\index{Lufträume}\index{Flarm}\index{Teamflug}
...
benutzt werden.: \todonum{Link des DAEC einfügen!}
Es können zwei Luftraumdateien in den Konfigurationseinstellungen benannt und benutzt werden,
welche auch beliebig bearbeitet werden können. Benutzt wird hierbei das \textsf{OpenAir}-Format.\index{Lufträume!Format}

\button{Konfig.}\blink\button{Konfig.}\blink\bmenut{System}{Einstellung}
...
\bmenu{Info}\blink\bmenut{Was ist}{hier?}\index{Was ist hier?}\blink\bmenu{Details}\blink\bmenu{Aktivieren}
...
\button{Warnung Bestätigen}, denn der Warnmechanismus hat keine Veranlassung
...
\button{Konf.}\blink\button{Konf.}\blink\button{Systemeinstellungen}\blink\button{Kartenanzeige}\blink\button{Luftraum}
...
\tip Wenn gewünscht ist, daß die Warnungen nicht wiederholt werden, setze die
 ...
 \index{Luftraumwarnung!Bestätigen!Bestätigungszeit}
...
\section{Luftraum Bestätigung im Voraus}\index{Luftraum!Bestätigen!Im Voraus}
...
Über den Button \button{Details} können so Lufträume im Voraus bei der Flugplanung
als bestätigt eingegeben werden, bevor man in die Nähe kommt und der
Warnmechanismus greift.
...
Lufträumen während des Fluges umgangen werden.

%Touching the status message or pressing the enter key makes the message disappear (that
%is, it acknowledges the query).  The query returns the details of all
%airspace regions when overlapping airspace is visible at the query location.

 Ein weiterer Weg, Lufträume im voraus zu deaktivieren kann über das
 Menü erfolgen:
 \button{Konf.}\blink\button{Konf.}\blink\button{Luftraumeinstellungen}\blink\button{Suche}
Hier kann bequem innerhalb einer flexiblen Suchliste jeder einzelne Luftraum in
seinen Details angesehen, bestätigt oder wieder aktiviert werden.
Siehe hierzu auch nächstes Kapitel (\ref{sec:airspace-filter}

\section{Luftraumfilter Dialog}\label{sec:airspace-filter}

Über den Luftraum-Filter Dialog können alle Lufträume gemäß Ihres Types im
Erscheinungsbild geändert werden.

Das kann über verschiedene Wege geschehen:
\begin{itemize}
\item Aus dem Hauptmenü:\\ \button{Konf.}\blink\button{Konf.}\blink\button{Luftraum Einstellung}
\item Aus den Konfigurationseinstellungen:\\  \button{Konf.}\blink\button{Konf.}\blink\button{System Einstellung}
\blink\button{Karteneinstellungen}\blink\button{Luftraum}\blink\button{Filter}
\end{itemize}

Hier können mit der Enter-Taste oder durch Berührung des entsprechenden Feldes diverse
Einstellungen und Warnverhalten eingestellt werden.

\begin{center}
\includegraphics[angle=0,width=0.7\linewidth,keepaspectratio='true']{figures/airspacefilter.png}
\end{center}

Drücken des ''Suchen''-Buttons läßt den Luftraumsuche-Dialog erscheinen.
...
\begin{quote}
\button{Info}\blink\button{Analysis}
\end{quote}
...
\todonum{wird das permanent aktualisiert?? Man könnte ja dies bei erneutem Flarmkontakt mit dem Piloten abgleichen ??}
...
\bmenu{Info}\blink\bmenu{Info}\blink\bmenut{Team}{kode}\index{Teamflug!Eingabe Team-Kode}
aufgerufen:
\begin{center}
