...
Da in diesem Beispiel beide Befehle \verb|file=| und \verb|image=|genutzt werden, besteht alternativ die Möglichkeit über den linken Pfeil-Button \button{$<$} direkt die Bilder zu erreichen, die über den Befehl \verb|image=| zur Anzeige im internen Viewer eingebunden wurden. 
\sketch{figures/dialog-waypointdetailsImageUebersicht.png

Hier wird dann zunächst das letzte Bild der Liste angezeigt, deshalb positioniert man sinnvollerweise die \verb|image=| -Bilder in der umgekehrten Anflug-Reihenfolge. (Wer nur mit der rechten Pfeiltaste blättern will, macht es besser in der korrekten Anflugreihenfolge, wie bei dem Befehl \verb|file=| - aber ganz nach belieben). 

Durch weiteres Blättern nach links werden dann die weiteren Bilder angezeigt (wie oben erwähnt nicht schwenkbar, aber über die Tasten zoombar zur Mitte hin).
