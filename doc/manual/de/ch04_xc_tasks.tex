...
Der Wegpunkt-Info-Dialog beschreibt die Details der Wegpunkte  und bietet Navigations-Optionen wie zum Beispiel ''Ziele auf'', ''Einfügen'' oder ''Anhängen'' an einer Aufgabe. Weiterhin kann über diesen Dialog ein Wegpunkt als Heimatflughafen eingegeben werden.
...
Der Wegpunktdetail-Dialog enthält zwei Basis-Seiten, welche über \button{$>$} und  \button{$<$} weiterschaltbar sind. Je nach dem, ob noch weitere Informationen zum Wegpunkt vorliegen (z.B. Text oder Bilder aus einer evtl. vorhandenen Wegpunkt/Flugplatz-Detail Datei, näheres weiter unten), erscheinen weitere Seiten mit Details hierzu.

Auf der ersten Seite werden Informationen (aus der Wegpunkt.cup Datei) angezeigt, wie z.B. Beschreibung, Richtung und Länge der Piste/Landefeld, Koordinaten, Höhe, Richtung und Distanz zum Wegpunkt etc.

Zusätzlich befindet sich unten ein Button \button{Fliege Zu} um direkt zu diesem Punkt zu navigieren.  Dieser Button bricht die aktuelle Aufgabe, sofern vorhanden, ab.

\sketch{{figures/dialog-waypointdetails0.png}}
Wie bereits oben erwähnt, zeigt der Wegpunkt-Info-Dialog auch drei Höhendifferenz-Angaben zum Erreichen des Wegpunktes, welche hier beschrieben werden:
...
Über die Buttons \button{$<$} und \button{$>$} am unteren Rand des Fensters können weitere Seiten mit Informationen über den aktuellen Wegpunkt angewählt werden. Weiterhin wird damit ein Menü geöffnet, auf dem z.B. dieser Wegpunkt als Ziel, Heimat oder Aufgabenwegpunkt eingefügt werden kann.

% Der folgende Punkt scheint nicht mehr aktuell zu sein:  
%
% >>
% \subsection*{Flugplatz Informationen}
% Diese Seite kann -sofern Daten zum Flugplatz 
% vorhanden- etliche Hinweise und Details zu 
% Flugplätzen enthalten. \sketch
% {figures/dialog-REM waypointdetails1.png}
% Insbesondere sind hier enthalten Details zur 
% Landebahn, Landebahnrichtung, Höhe, Frequenz usw.
% <<
%
% streichen?

\subsection*{Anzeige von Bildern etc.}
Auf den weiteren Seiten können eigene Bilder angezeigt werden, z.B. eine Grafik mit Anflugbeschreibung, Foto einer Landewiese usw. Näheres hierzu siehe auch Kap.~\ref{sec:airfield-details} 
Das Bild kann dabei mit den Buttons + und - oben auf der Seite gezoomt werden, allerdings nur in zentraler Richtung.
\sketch{figures/dialog-picture-int.png}

Falls eingerichtet können mit einem Default-Viewer, der auf dem Gerät installiert sein muss, Bilder frei zoombar angezeigt werden. Über diesen Weg können auch unterschiedlichste Mediendateien (beispielsweise mehrseitige .pdf, .bmp, tif, mp4 Dateien u.v.m.) direkt aus XCSoar heraus genutzt werden. Details dazu sind im Kap.~\ref{sec:airfield-details} beschrieben.
