...
in einer der mailing-listen  (z.B.:  im \href{https://forum.xcsoar.org}{XCSoar-Forum}) um Hilfe bitten.
...
\begin{description}
\item[\textit{Quelle der Lokalzeit}] {\bf Automatisch / Zeitzone / Manueller UTC-Offset}: Legt fest, woher \xc den Offset zwischen UTC und Lokalzeit bezieht.\\
{\bf Automatisch}: Der Offset wird aus der im Betriebssystem eingestellten Zeitzone übernommen und laufend nachgeführt. Damit folgt er automatisch der Sommer-/Winterzeitumstellung und einem Wechsel in eine andere Zeitzone. Auf dem Kobo steht diese Einstellung nicht zur Verfügung, weil das Gerät gar keine Zeitzone kennt.\\
{\bf Zeitzone}: Der Offset wird aus der unten gewählten Zeitzone berechnet. Eine Zeitzone enthält die Regeln für die Sommerzeit, \xc schaltet also selbst um. Die Einstellung muss nicht zweimal im Jahr angefasst werden.\\
{\bf Manueller UTC-Offset}: Es wird der unten von Hand eingegebene Offset benutzt. Er muss zu jeder Zeitumstellung korrigiert werden. Diese Möglichkeit wird nur im Expertenmodus angeboten, ausser sie ist gerade eingestellt.
\item[\textit{Zeitzone}] Die Zeitzone des Flugplatzes, auf dem geflogen wird, z.\,B. ''Europe/Berlin``. Das Feld ist nur bearbeitbar, wenn als Quelle der Lokalzeit ''Zeitzone`` gewählt ist.
\item[\textit{Manueller UTC-Offset}]  {\bf -13 ..+14h}: In diesem Feld wird der Offset der Lokalzeit zur UTC-Zeit eingegeben. Das Feld ist nur bearbeitbar, wenn als Quelle der Lokalzeit ''Manueller UTC-Offset`` gewählt ist, und behält den eingegebenen Wert, solange eine andere Quelle gewählt ist. Die Differenz kann im 15-Minuten-Intervall eingegeben werden.
...
\item[\textit{Lokalzeit}]  Das ist die oben errechnete Lokalzeit -- nur zur Kontrolle. Dahinter steht in Klammern der gerade gültige UTC-Offset.
