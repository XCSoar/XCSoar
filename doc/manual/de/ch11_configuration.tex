...
Es gibt eine Menge an Einstellungen, welche vom Benutzer verändert werden
...
wenn kein File benutzt bzw. angelegt werden muß, das Auswahlfeld einfach leer.
...
Der Pfad kann sich auf der Festplatte, der SD-Karte oder im ''normalen'' Speicher
...
\item[\textit{Wegpunkte}] Das primäre Wegpunkt-File. 
\item[\textit{Weitere Wegpunkte$^{\textcolor{blue}{\star}}$}]  Eine zweite Wegpunktdatei. Hier können z.B. Wegpunkte und spezielle Angaben für Wettbewerbe oder ähnliches hinterlegt werden.
\item[\textit{Beobachtete Wegpunkte$^{\textcolor{blue}{\star}}$}] Eine dritte Wegpunktdatei. Sinnvoll für z.B.\ bekannte und zuverlässige Thermikauslösepunkte, Bergrücken, Kraftwerke etc.\

Grundsätzlich gilt: Wenn in keinem der drei Slots ein file ausgewählt ist, werden die Wegpunktdaten aus der Kartendatenbank (\verb"*.xcm"), s.o.\ benutzt (sofern dort vorhanden).


\item[\textit{Lufträume}]  Primäre Luftraumdatei. 
...
Wenn in keinem der beiden Slots ein file ausgewählt wird, werden die Daten aus der Kartendatenbank (\verb"*.xcm"), s.o.\ benutzt (sofern dort vorhanden).

\item[\textit{Wegpunktdetails$^{\textcolor{blue}{\star}}$}]  Hier kann eine Datei angegeben werden, in der ergänzende Details zu Wegpunkten, Landewiesen oder Flugplätzen enthalten sind. In dieser Datei können auch Bilder und andere Medienfiles eingebunden werden. Weitere Informationen zur Einrichtung sind im Kap.~\ref{sec:airfield-details} beschrieben.
\end{description}
...
Karten wie Topologie, Gelände und Topographie einzugeben. Vor der Version 6.4 von \xc waren  hierzu noch
andere Files anzugeben, die explizit die Topologie bzw. das Gelände enthielten.
...
Wenn diese Files im XCM File vorhanden sind, dann muß keine separate Wegpunktdatei angewählt werden und \xc nimmt die Wegpunkte aus der XCM-Datenbank.
