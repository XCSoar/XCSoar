...
\item[\textit{Weitere Wegpunkte$^{\textcolor{blue}{\star}}$}]  Eine sekundäre Wegpunktdatei. Hier können z.B. Wegpunkte und spezielle Angaben für Wettbewerbe oder ähnliches hinterlegt werden.
\item[\textit{Beobachtete Wegpunkte$^{\textcolor{blue}{\star}}$}] Eine tertiäre Wegpunktdatei. Sinnvoll für z.B.\ bekannte und zuverlässige Thermikauslösepunkte, Bergrücken, Kraftwerke etc.\

Grundsätzlich gilt: Wenn in keinem der drei Slots ein file ausgewählt wird, werden die Wegpunktdaten aus der Kartendatenbank (\verb"*.xcm"), s.o.\ benutzt (sofern dort vorhanden).
...
Wenn in keinem der beiden Slots ein file ausgewählt wird, werden die Wegpunktdaten aus der Kartendatenbank (\verb"*.xcm"), s.o.\ benutzt (sofern dort vorhanden).

\item[\textit{Wegpunktdetails$^{\textcolor{blue}{\star}}$}]  Hier kann eine Datei angegeben werden, in der ergänzende Details zu Wegpunkten wie z.B. Landewiesen oder Flugplätzen enthalten sind. Details zur Einrichtung sind im Kap.~\ref{sec:airfield-details} beschrieben.
\end{description}
