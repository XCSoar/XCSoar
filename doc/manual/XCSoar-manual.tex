...
\todonum[inline]{support more task types}
\todonum[inline]{alternates list}
\todonum[inline]{instant OLC score}
\todonum[inline]{OLC plus rules}
\todonum[inline]{instant AAT optimization}
...
color. The Quickstart will be readable also without those links available} 
...
\todonum[inline]{explain the principle display modes}
...
\todonum{verify} \jindent{\bmenut{Advance}{(Auto)}}{ Arms the automatic task 
waypoint trigger. }
\todonum[inline]
{add (or not?) to dynamic buttons: {Abort}{Start} - { Abort the
last Start } } 
...
\todonum{explain} \jindent{\bmenus{Alternates}}{  }
...
\jindent{\bmenut{Full Scrn}{On}}{ Toggles the full screen map display. }
...
\jindent{\bmenut{Labels}{On}}{ Displays map labels, MID lables, or none. }
...
\todonum{verify restriction to final glide} \jindent{\bmenut{MC}{Auto}}{ Toggles
automatic/manual MacCready when in final glide. }
...
the waypoint nearest to the aircraft. }
...
\jindent{\bmenut{Aux Info}{On}}{ Toggles the infobox display between normal
(flight-mode specific) or auxiliary infobox display. }
...
dialog. }
\todonum{the thermal assistant should beexplained somewhere}
...
\jindent{\bmenut{Marks}{clear}}{ Clear all previously propped marks. }
...
\begin{quote}
\bmenu{Display}\blink\bmenu{Full Scrn On}
\end{quote}
...
menu \bmenu{Config}\blink\bmenu{Setup System} or by performing a long press on
...
in or out.
\item You can also gesture to change the zoom level. Gesture \gesture{Up
and Down} ``up'' zooms in, ``Down'' zooms out.
...
\begin{quote}
\bmenu{Display}\blink\bmenu{Pan Off}
\end{quote}
...
\hline
Purple Circle &
\includegraphics[width=0.5cm]{icons/map_turnpoint.png} &
\includegraphics[width=0.8cm]{icons/winpilot_landable.png} &
\includegraphics[width=0.8cm]{icons/winpilot_reachable.png} &
\colorbox{white}{\includegraphics[width=0.8cm]{icons/winpilot_landable.png}}
& \includegraphics[width=0.8cm]{icons/winpilot_reachable.png} \\
\hline
B/W Icon & 
\includegraphics[width=0.5cm]{icons/map_turnpoint.png} &
\includegraphics[width=0.9cm]{icons/alt_landable_field.png} &
\includegraphics[width=0.9cm]{icons/alt_reachable_field.png} &
\colorbox[rgb]{0.94,0.94,0.94}{\includegraphics[width=0.9cm]{icons/alt_landable_airport.png}}
& \includegraphics[width=0.9cm]{icons/alt_reachable_airport.png} \\
\hline
Orange Icon & 
\includegraphics[width=0.5cm]{icons/map_turnpoint.png} &
\includegraphics[width=0.9cm]{icons/alt2_landable_field.png} &
\includegraphics[width=0.9cm]{icons/alt_reachable_field.png} &
\colorbox{white}{\includegraphics[width=0.9cm]{icons/alt2_landable_airport.png}}
& \includegraphics[width=0.9cm]{icons/alt_reachable_airport.png} \\
\hline
\end{tabular}
...
minimum MacCready setting \todonum{explain somewhere else in detail: the MC value scaled according to your alitude compared with the cloud base
altitude
}.
...
vertical movement \todonum{verify}.
...
circling mode in order to \todonum{verify} reduce screen clutter.
...
\todonum[inline]{this all will go}
...
details. \todonum{not correct, research and updated}
...
\todonum{Needs a bit more functionality here}
\end{itemize}
...
\section{OnLine Contest}
  ...
  configuration settings \config{taskrules} (task rules page) allows the
  ...
  of this start point can be \todonum{update} enabled in the task editor,
...
\todonum[inline]{Describe how XCSoar interfaces with it\ldots}
...
for altitude required to complete the task. \todonum{explain better}
\item[Task speed remaining]  This is the task speed estimated for the
  remainder of the task.\todonum{doublicate definition, explain better}
...
LX1600 variometers, Zander, ILEC SN10, Tasman Instruments variometer, Flymaster
F1, Flytec, amd Leonardo. 
...
height above the break-off \todonum{update} height, and is scaled according to
...
\todonum[inline]{explaine the thermal assistant}
...
divided by time spent thermaling.}
...
\ibi{Forecast Temperature}{MaxT}{Forecast temperature of the ground at the
...
\ibi{Speed Task Average}{V Task Ave}{Average cross country speed while on
current task, compensated for altitude.}
\ibi{Speed Task Instantaneous}{V Task}{Instantaneous cross country speed while
on current task, compensated for altitude.}
...
\ibi{Own Team Code}{TeamCode}{The current Team code for this aircraft. Use this
to report to other team members.}
\ibi{Team Bearing}{Team B}{The bearing to the team aircraft location at the last
team code report.}
\ibi{Team Bearing Diff}{Team B D}{The relative bearing to the team aircraft
...
zoom levels will be maintained for circling and cruise modes.  If unchecked, there
...
\todonum{trail drift: measuring unit?}
...
display.
\item[Detour cost]  \todonum{explain}
...
\item[Idle blank screen] (PDA only) \todonum{move}
...
\todonum[inline]{The check boxes in this section determine whether the compass
indicator will be displayed in cruise/final glide modes and circling mode.}

%%%%%%%%%%%%%%%%%%
\clearpage
\section{Symbols}\label{sec:symbols}
\todonum[inline]{Complete Symbols page.}

This page provides options relating to the display items overlayed map.
...
\item[Final glide bar] \todonum{update} Two styles are available: Default and
Alternate. The differences between these styles is cosmetic.  Alternate displays the height difference to the right of the final glide bar; default displays the height difference above/below the final glide bar and inside a rounded box.
\item[Landable fields] \label{conf:waypointicons} Three styles are available:
Purple circles (WinPilot style), a high contrast (monochrome) style with icons, or orange icons.
...
\todonum{auto-scale terrain colors}
\end{description}
...
\todonum[inline]{ scheme ``grey'' is missing; ``flatlands'' is displayed
``lowlands''}
...
\todonum[inline]{dialog style: description of values}
...
other languages.  If this field is left blank, \todonum{update, language
auto-detection} then XCSoar uses English.
\item[Status message*]
...
\item[ThermalAssistant] \label{conf:thermalassistant}
Enables the display of the ThermalAssistant gauge.
\end{description}
...
%  start line.
%\item[Sector type]  Cylinder, FAI sector or German (DAeC 0.5/10) sector
%\begin{description}
...
%\item[DAeC 0.5/10] German national sector type, equivalent to 0.5 km radius barrel and 10 km FAI sector.
%\end{description}
...
\begin{description}
\item[OLC Sprint]  \todonum{update}Conforms to FAI IGC League rules.  Up to 5
points including start and finish, maximum duration 2.5 hours, finish height must not be below start height.
\item[OLC FAI]  \todonum{update}Conforms to FAI OLC triangle rules.  Four points
...
\item[OLC Classic]  \todonum{update}Conforms to OLC classic rules.  Up to seven
...
\todonum[inline]{ describe use of profile files at startup (new selector)
  and how to create a new profile for shared aircraft; or for aircraft
  with different configurations (wingtips etc.)}

\todonum[inline]{When a profile is selected at startup, it becomes the default
profile?}
