...
says \bmenu{Auto MC ON}, then pressing the button will turn auto
MacCready on, and the button label will then change to \bmenu{Auto MC
...
\jindent{\bmenut{Previous}{turnpoint}}{ Selects the previous waypoint in the
task }
\jindent{\bmenut{Next}{turnpoint}}{ Selects the next waypoint in the task }
...
\jindent{\bmenus{Bright}}{ Adjusts screen brightness (only for Altair) }
\jindent{\bmenut{Declutter}{Labels}}{ Toggles display of topology and non-task 
labels }
...
\jindent{\bmenut{MacCready}{Auto}}{ Toggles automatic MacCready when in final
glide. }
...
recorder } 
\jindent{\bmenut{Logger}{Replay}}{ Displays the IGC/NMEA logger replay dialog }
