...
When Track-up is used in circling mode, the glider symbol is
centered on the screen, even if the symbol position is configured differently.
In cruise mode the Track up orientation allows the glider symbol to be 
positioned (e.g.) 20\% from the bottom of the screen, giving a good view of the
...
in circling mode, and one in the cruise or final mode.  This is the "Circling zoom" 
option in the configuration settings.  
By default, the cruise/final glide zoom is \todonum{check} 5 km and the
circling zoom is \todonum{check}2.5 km.
...
zoom setting is used.  If `Circling Zoom' \config{circlingzoom} is not enabled,
...
\todonum{what is a ``waypoint number''?}\item[Numbers] The waypoint number of
