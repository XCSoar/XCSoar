...
Android and iOS devices have a built-in GPS.  Many of those phones and
tablets also have a barometric pressure sensor; XCSoar uses it for
altitude when no better external altimeter is connected.  On Android,
XCSoar also uses the phone's accelerometer as a G-sensor (load
factor).  For several reasons it is still favorable to connect to one
...
Modern Android devices typically have no serial port.  Those can either
...
\texttt{Documents/XCSoarData} and can be copied with the Files app
(or iTunes file sharing).

\subsection*{On macOS}

Install XCSoar from the Apple App Store, or download the macOS package
from the XCSoar website.  Data files go in \texttt{XCSoarData} in your
home folder.
...
\begin{itemize}
\item \textbf{64-bit} (target ``WIN64OPENGL''): the recommended
...
\item \textbf{32-bit} (target ``WIN32OPENGL''): same as the 64-bit
  build for 32-bit Windows; installer or \verb|.zip| available.
\end{itemize}

The version is visible in Windows file properties of
\texttt{XCSoar.exe} and the installer.
...
Unix/Linux builds using software rendering with SDL require SDL3 version
3.2.10 or newer. Install the SDL3 runtime supplied by your distribution;
SDL2 is no longer sufficient for these builds. The standard OpenGL build
is unaffected.
...
Documents}'')
\item[macOS]
\texttt{XCSoarData} is in your home folder.
\item[iOS]
\texttt{XCSoarData} is in the app's Documents folder, accessible via
the Files app.
\item[Unix/Linux]
...
will be placed in the  \texttt{XCSoarData} directory (Windows, macOS,
iOS, and Android devices), or the \texttt{.xcsoar} directory (Unix/Linux
...
\item[SIM] This starts XCSoar in simulator mode, no serial communications
  are attempted.
\end{description}
Quitting from this prompt leaves the profile file unchanged, including
when XCSoar was started with \verb|-profile=...|.
...
XCSoar is shut down from the menu. The menu can be
