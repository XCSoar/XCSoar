...
compatible with version 4.4, because that is what \texttt{cegcc} is
based on.
...
\item Explicit conversion operators (with macro \verb|gcc_explicit|)
\item Strongly-typed enums
\item Defaulted and deleted functions
\item Generalized constant expressions (\verb|constexpr|): see below
\end{itemize}

\subsubsection{Generalized constant expressions}

The keyword \verb|constexpr| is supported by GCC since version 4.6.
It can be applied to functions, methods, constructors and variables.

On functions, it replaces our macro \verb|gcc_const|.  Methods are
never \verb|gcc_const|, because they usually dereference the
\verb|this| pointer.  Attributes marked with \verb|constexpr| are
implicitly \verb|const|, and you may not use the latter keyword then.

Until we drop support for GCC versions older than 4.6, we need to have
a fallback for older compilers.  To solve the different requirements
for different ways to use \verb|constexpr|, we have created three new
macros in \verb|Compiler.h|:

\begin{itemize}
\item \verb|gcc_constexpr_function| for functions. Falls back to
  \verb|gcc_const|.
\item \verb|gcc_constexpr_method| for methods. Falls back to
  \verb|gcc_pure|.
\item \verb|gcc_constexpr_ctor| for constructors. Falls back to
  nothing.
\item \verb|gcc_constexpr_data| For variables. Falls back to
  \verb|const|.
\end{itemize}
...
\subsection{Dialog types and navigation buttons}
...
\begin{itemize}
\item Dialogs that modify and save data when the dialog closes.
...
children dialogs where hitting Cancel from the parent dialog cancels 
all the changes made in the children dialogs

\item Dialogs that have a list of values, one of which can be selected to return to the parent dialog.
...
\subsection{Dialog button placement and size}
\begin{itemize}
\item The Close and Cancel buttons will never appear in the same dialog and are always located in the same place.  This location will be:
...
\item Buttons will be flush with the bottom of the screen and with the sides of the screen and against each other (no margins)
\item In portrait, buttons will be 33% or 50% or 100% width of the screen
\item In landscape, buttons will be 65 to 80 (scaled) pixels wide, as wide as the frame permits.  They will generally be a vertical row of buttons flush left of the screen
\item If text won't fit on a button, the buttons can be made larger consistently for a screen, but this should be the exception because if it must contain that much text consider using a different type of control.
\item Exceptions to all the dialog concepts above are encouraged, but should be mocked up and reviewed with the development community prior to implementing and possibly documenting in the developers guide.
\end{itemize}
