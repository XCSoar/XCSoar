...
configuration page, \ref{sec:site-files}. 
...
The current field name is displayed in the lower left corner of the 
screen whenever RASP data is displayed.
...
The EDL controls are available from the
\menulabelr{\bmenug{Info}\blink\bmenug{Weather}} dialogue on the
\bmenuw{EDL} tab.  This tab allows the pilot to:
\begin{description}
...
\item[Always show EDL on map] Keep the EDL overlay visible on ordinary
  map pages.  When this option is disabled, the overlay is shown only on
  the dedicated EDL page.
...
\item[Precache day] Download all 24 hours and all five EDL levels for
  the currently selected UTC day.  This is intended for pre-flight
  preparation, so that the EDL data remains available when internet
  connectivity is lost in flight.
\item[Clean other days] Keep only the selected cached UTC day and delete
  the other locally cached EDL days.
\end{description}

EDL overlays can be used in two ways:
\begin{description}
\item[Dedicated EDL page] In the page configuration dialogue
  (Section~\ref{conf:screenpages}), select \emph{EDL weather} as the
  \emph{Main area} and \emph{EDL} as the \emph{Bottom area}.  Entering
  this page resets the display to the current UTC hour and selects the
  nearest supported EDL level for the current glider altitude when that
  altitude is available from the connected sensors.  The overlay legend
  in the upper right corner shows the active local forecast hour, the
  selected pressure level and its approximate altitude.
\item[Overlay on the normal map] Open
  \menulabelr{\bmenug{Info}\blink\bmenug{Weather}} \bmenuw{EDL}, select
  the desired forecast hour and level, and enable
  \bmenuw{Always show EDL on map}.  The normal moving map remains in use,
  with the EDL weather image shown as an overlay on top of the map.
\end{description}
