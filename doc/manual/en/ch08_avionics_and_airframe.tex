...
The same is true for values such as barometric altitude (QNH),
pressure altitude (QNE), logger altitude (IGC), vario and
airspeed: XCSoar prefers the first device, and uses the second
device only for values that are not received from the first one.
Traffic (FLARM) from every device that reports it is used.
