...
\item[Serial device]  Certain intelligent variometers such as
 ...
 NMEA sentences.
\item[Bluetooth device]  Many devices support wireless
...
and then selecting the tabular \textbf{System}.
...
\menulabel{\bmenug{Config 1}\blink\bmenug{Devices}}
...
\texttt{XCSoarData}.  A flight download shows the flight name, the connected device, and the
driver on separate lines above the progress bar.  Inside the bar it
shows the percentage complete, the amount received against the total,
the transfer speed, and an estimate of the time remaining.
