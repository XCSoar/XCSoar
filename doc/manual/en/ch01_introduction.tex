...
computer originally developed for handheld devices.  The audience 
...
XCSoar runs on Android 6.0 or newer.  Many phones and tablets have a
built-in GPS, a barometric pressure sensor, and an accelerometer
(G-load) that XCSoar can use.
\item[iOS]
XCSoar runs on iPhone and iPad. Install it from the Apple App Store.
Many of these devices have a barometric pressure sensor that XCSoar
can use when present.
\item [eBookreader]
...
\item[macOS]
XCSoar runs on Apple Mac computers. Install it from the Apple App Store
or from the XCSoar website.
\item[Unix/Linux PC]
