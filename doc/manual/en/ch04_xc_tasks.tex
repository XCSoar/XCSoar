...
Note that in goto and abort modes, the ordered task is retained an may be resumed
...
the waypoint list, or any other mechanism (e.g.\ alternate dialog),
...
points, ordered according to one of three `Alternates mode'\config{alternatesmode} configuration settings:
\begin{description}
\item[Simple] The estimated arrival altitude at best glide angle
with the safety MacCready ring setting, adjusted for wind.    
The first waypoint in the list is therefore the most reachable waypoint.
\item[Task] The sorting will also take the current task direction into account, such that the sort order is according to minimum extra distance travelled to the respective field and onwards to the task.
\item[Home] The sorting will try to find landing options in the current direction to the configured home waypoint.  Similar to `task' but with the home field as the desired destination.
\end{description}
