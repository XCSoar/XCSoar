...
platform (Android, iOS, Windows, Linux, macOS\textellipsis).
...
The airfields or waypoint details dialogue contains two major pages (accessed via the
...
It is a good idea to set your home waypoint from the airfields or waypoint details
...
\item[Symmetric Sector] A Symmetric Sector of a circle with a custom
...
line that is its centre or one of its ends.  While the option is set,
it aims at the nearest point of the zone instead, which moves as the
glider moves, so the distance, the estimated time and the arrival
height describe the place the glider is going to cross.  The bearing
line on the map, the airspace warnings along the leg and the waypoint
sent to devices which receive the next waypoint (for example a CAI
L-Nav or an LX160) follow that point too.  Task distance, its
optimisation and scoring are unchanged.  A sector is left alone, and so is a cylinder the glider is
already inside, where the nearest point of the rim is the wrong way
out.  On a start cylinder the distance and the estimates therefore
change once as the glider enters it.
...
\bmenug{Alternates}} Up to ten landing
...
\button{Start} button.  The file selector lists IGC and NMEA logs.  IGC
files show detected takeoff, landing and duration as the list is scanned.
The number of IGC files with a detected launch and landing, and their
total time as hours:minutes, are shown under the list.
Leave the file blank, or choose Demo, to run the demo.  The replay can be
performed in accelerated time
