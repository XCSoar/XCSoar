...
\begin{itemize}
\item Pressing \bmenu{Nav}\blink\bmenu{Nav}\blink\bmenu{Task Abort}
...
Alternates are maintained throughout the flight. They are filtered by
the configured `Alternates mode' criteria. The first two items of that 
alternates list are available as InfoBox. 
\config{alternatesmode} (Simple, Task, or Home).
Althouth the alternates obey different rules they interact in the same way 
with the current task. Choosing a target of the alternates list aborts the task; 
