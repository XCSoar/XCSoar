...
\ibi{Blood Oxygen}{SpO2}{Blood oxygen saturation in percent from a Bluetooth
pulse oximeter on Android. At 90\% and above the value uses the normal text
colour, from 85\% to 89\% it is amber, and below 85\% it is red. After 30
seconds the comment line shows the age of the reading. The value is cleared
after three minutes.}
...
\ibi{Thermal next leg equivalent}{T Next Leg}{Climb on the next task
leg that gives the same average cross-country speed as flying the
current leg at the MacCready setting, using the polar and the headwind
on each track.  The box is invalid when there is no next leg, wind, or
polar.  The comment shows ``no wind'' when wind is not available.
See Section~\ref{sec:t-next-leg}.}
