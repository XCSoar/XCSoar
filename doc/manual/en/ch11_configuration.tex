...
%%%%%%%%%%%%%%%%%%
\section{Look / Display}

This page sets how XCSoar draws on the screen.

\begin{description}
\item[Display type*]  The display technology.  E-ink modes disable
  kinetic and smooth scrolling on slow refresh screens, draw text
  without anti-aliasing (the same 1-bit glyphs as on Kobo), and use
  thin body text and stronger emphasis on Android so buttons stay
  light while QuickGuide bold and headings remain visible on color
  e-paper panels.  Text size follows the same density and Text size
  setting as LCD; only rasterisation changes.
\item[Display resolution*]  Override the automatic density used for
  line widths, font size, and map symbols.  Leave this on automatic
  unless the type is clearly the wrong size.
\item[Display orientation]  Rotate the display on devices that support
  it.
\item[Dark mode]  Night colours, or follow the operating system.
\item[Text size]  Scales all UI text.  At 100\%, type follows the
  display density.  Pocket screens whose long edge is under 5 inches
  use a closer-view size; 6 inch phones keep full-size type.
\end{description}
