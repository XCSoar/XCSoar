...
\item[Port]  This setting maps an existing interface (port) of your 
  ...
  and {\it Baro} are drawn in bold; the same flag in regular weight
  on a later device is present but ignored.
  ...
  10~percent.
  {\it FLARM} is bold while that device's FLARM status is fresh
  (a recent PFLAU heartbeat), and {\it GDL90} is bold while the
  protocol is alive; leftover targets alone are not enough.
