...
\item[Contours*]  Draws contour lines on the terrain.  The selected mode
  sets the base line density, which is also scaled automatically with the
  zoom level to avoid over-cluttering at small scales.
  Four different modes are provided for different landscape types. {\bf Mountains}
  is for areas with steep slopes, with a 256\,m base line spacing. 
  The other modes provide progressively smaller contour spacing and higher line density.
\end{description}
...
\item[Bottom area]  Optional panel below the main area.  Select
  \emph{Weather controls} to show in-flight RASP or EDL forecast controls
  when the page uses a weather overlay.
\item[Map overlay]  Optional weather overlay drawn on map pages: \emph{None},
  \emph{RASP}, or \emph{EDL}.  RASP requires a loaded RASP file; EDL
  requires \emph{EDL Weather Page} to be enabled in the weather system
  configuration.
\item[RASP layer]  When \emph{Map overlay} is \emph{RASP}, selects which
  RASP field is displayed on this page.
\end{description}
