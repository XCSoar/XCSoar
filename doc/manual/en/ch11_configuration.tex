...
\item[Black cursor] Use black instead of white cursor. Default is {\bf Off} (white cursor).
  Only available if cursor is drawn by XCSoar.
\item[Scroll bars]  Determines how lists and long dialogues show their
  scroll bar. {\bf Automatic} lets the device decide: on a touch
  screen the content is dragged, so the bar only shows while
  scrolling; a device operated by keys or a knob cannot press arrow
  buttons at all, so it gets the slim bar, which has none; everything
  else gets the standard scroll bar. {\bf When scrolling} shows a thin
  indicator on top of the content while it is being scrolled and hides
  it afterwards, which leaves the full width to the content.
  {\bf Slim} puts a thin bar beside the content, always visible: it
  shows the position at a glance, has no arrow buttons and takes only
  a narrow column. {\bf Standard} keeps the scroll bar with arrow
  buttons beside the content, always visible.
\end{description}
