...
\item[Full screen]  Let XCSoar use the whole screen, including the areas
  ...
  always fills it completely; \emph{Stretch safe area} decides which edges
  the InfoBoxes, gauges and map overlays may reach.
  Only available on Android and iOS.
\item[Stretch safe area*]  The safe area is the part of the screen that is
  available to the InfoBoxes, gauges and map overlays. Selecting this entry
  opens a picker in which you choose the screen edges on which the safe area
  is stretched out to the screen border, so that they reach the very edge
  there. Dialogues, the menu buttons and the pages shown instead of the map
  always stay inside the safe area.
...
\item[System status bar]  Whether the system status bar with the clock and
  ...
  {\bf Auto}: Show it unless the safe area is stretched to the top screen
  edge. \\
