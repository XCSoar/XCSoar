...
\item[Trail drift*] \label{conf:traildrift} Determines whether the
  ...
  the snail trail stays uncompensated for wind drift.  In circling this also applies
  to the projected track curve: On keeps it relative to the air mass; Off includes wind.
...
\item[Waypoint icon size] Size of waypoint symbols on the map as a percentage of
  the built-in artwork. List dialogs keep a fixed row icon size.
...
\item[Reach mode]  \label{conf:turningreach} How calculations are performed of 
  ...
  {\bf Off}: Disables map reach (terrain line and shade). Abort, alternates
  and landable arrival still use the Reach polar. \\
...
\item[Reach polar*]  \label{conf:reachpolar} This determines the glide performance 
  used in map reach, landable arrival, abort and alternate calculations.
  This still applies when Reach mode is Off. \\
...
%%%%%%%%%%%%%%%%%%
\section{Gauges / Traffic} \label{sec:flarmandother-gauge}

\begin{description}
\item[FLARM radar]  \label{conf:flarmdisplay}\label{conf:flarmradar-place} Enables the traffic radar and
  chooses its position on the main screen, or {\bf Off} to hide it.
  The track bearing of the target relative to the track bearing of the 
...
This page configures the analog vario gauge (shown with InfoBox layouts
that include a vario).
...
\item[Thermal Averager needle*]  If true, the vario gauge will display a thermal
  averager needle instead of the current climb-rate needle. During cruise, this
  needle displays the last thermal average net value. During circling, this
  needle displays the average net value.
\end{description}
...
%%%%%%%%%%%%%%%%%%
\section{Look / Screen}\label{sec:look-screen}

This page configures the physical screen: technology, resolution,
orientation, brightness, and (where drawn by XCSoar) the mouse cursor.

\begin{description}
\item[Display type*]  Select the display technology. E-ink modes disable
  kinetic and smooth scrolling for slow refresh screens.
\item[Display resolution*]  Used to adapt line widths, font size, landable
  size and more.
\item[Display orientation]  Rotate the display on devices that support it.
\item[Screen brightness]  Adjust the screen brightness, where the platform
  allows it.
\item[Full screen]  Run XCSoar in full screen mode (Android).
\item[Cursor zoom]  Control the size of the cursor. Default is 1.
  Only available if the cursor is drawn by XCSoar.
\item[Invert cursor color]  Use a black cursor. Default is {\bf Off}
  (white cursor). Only available if the cursor is drawn by XCSoar.
\end{description}


%%%%%%%%%%%%%%%%%%
\section{Look / Layout}\label{sec:interface-appearance}
...
%%%%%%%%%%%%%%%%%%
\section{Setup / Language, Input}\label{sec:interface}
...
%%%%%%%%%%%%%%%%%%
\section{Setup / Audio}\label{sec:audiovario-gauge}

Master volume and the audio variometer. \label{conf:audiovariogauge}

\begin{description}
\item[Master Volume]  The overall audio output volume.
\item[Audio Vario]  Enable/Disable the audio variometer.
\item[Vario Volume]  Set the volume of the variometer sounds.
\item[Mode switching]  Stay in manual audio mode, or switch automatically
  between Vario in circling and speed-to-fly in cruise.
...
`{\em SkyLines}' is a spin-off project from XCSoar itself; for details on this
service have a look at \url{https://skylines.aero}.

\begin{description}
\item[Enable]  Switch this {\bf On} to enable the live stream of 
...
%%%%%%%%%%%%%%%%%%
\section{Services / LiveTrack24}

The `{\em LiveTrack24}' protocol is used e.g.\ by the tracking portal on
\url{http://www.livetrack24.com}.
