...
\item[FLARM traffic]  \label{conf:flarm-on-map} This enables the display of FLARM 
  traffic on the map.
\item[Traffic symbol]  \label{conf:traffic-symbol} Determines how FLARM, ADS-B
  and online traffic is drawn on the map and on the traffic radar. \\
  {\bf Arrow}: All traffic is drawn as an arrow head pointing in the direction
  of travel. \\
  {\bf Aircraft type}: Traffic is drawn as a glider, powered aircraft,
  helicopter, paraglider or other symbol according to the aircraft type
  reported by the traffic source.  Targets of unknown type get a generic
  chevron symbol.
...
\item[Trail drift*] \label{conf:traildrift} Determines whether the
  snail trail is drifted with the wind when displayed in circling mode.  Off by
  default.  Switched Off, the snail trail stays uncompensated for wind drift.
\item[Trail type*] \label{conf:snailtype} Sets the type of the snail trail display. \\
  ...
  lines while sinking. \\
  {\bf Vario \#2} (default): The climb colour for this scheme is orange to red,
...
%%%%%%%%%%%%%%%%%%
\section{Map Display / Airspace}\label{sec:map-airspace}
...
\item[Label visibility*]  Shows the ceiling and base of each airspace
  on the map. \\
  {\bf None}:  No labels. \\
  {\bf All}:  A label on every displayed airspace.
...
can be used to review or change the colours used by each
...
First select the airspace class you wish to change. Then select the colour
you wish the selected airspace class to be drawn in.
...
\item[Haptic feedback*]  Let you switch on or off the vibration
  when the device accepts a finger press.  Available on Android
  and iOS.
\end{description}
...
\begin{description}
\item[Rename]  Sets the name of the currently customized InfoBox set.
  The button starts the text input dialogue of XCSoar. Only the sets
  you have made yourself can be renamed.
\item[Help]  Explains how an InfoBox is moved and how it is replaced.
\item[Copy Set]  Remembers all InfoBoxes of this set.
\item[Paste Set]  Replaces the InfoBoxes of another set with them.
\end{description}

See Section~\ref{cha:infobox} for a description of the InfoBox types and their meanings.

To change the content of an InfoBox in the set, long-press its card.
Without a touch-screen, press Enter twice without moving the selected
InfoBox.
...
\begin{description}
\item[Local time source]  Selects where XCSoar gets the offset between UTC and local
  time from:
  \begin{description}
  \item[Automatic]  The offset is taken from the time zone configured in the operating
    system and is kept up to date, so it follows daylight saving time changes and
    travelling to another time zone.  This option is not available on Kobo, because
    those devices have no time zone setting at all.
  \item[Time zone]  The offset is calculated from the time zone selected below.  A time
    zone includes the daylight saving time rules, which means XCSoar applies the
    transitions on its own; the setting does not have to be corrected twice a year.
  \item[Manual UTC offset]  The fixed offset entered below is used.  It has to be
    corrected manually whenever daylight saving time begins or ends.  This option is
    offered in expert mode only, unless it is the one in use.
  \end{description}
\item[Time zone]  The time zone of the airfield you are flying at, e.g.
  `Europe/Berlin'.  It is only editable while the local time source is `Time zone'.
\item[Manual UTC offset]  The field allows the UTC local time offset to be specified.
  It is only editable while the local time source is `Manual UTC offset', and it keeps
  the value you entered while another source is selected.
\item[Local time]  The local time is displayed below in order to make it easier to verify
  the correct offset has been entered, followed by the UTC offset which is currently in
  effect.  Offsets to the quarter-hour may be set.
...
\item[Port]  This setting maps an existing interface (port) of your 
  ...
  On Android, Bluetooth LE devices appear as {\it BLE port} (a
  serial UART used by varios such as BlueFly) or {\it BLE sensor}
  (heart-rate monitors, pulse oximeters, SensBox, and engine sensors).
  A BlueFly Vario must be selected as {\it BLE port} with the BlueFly
  driver; {\it BLE sensor} cannot carry its serial protocol.
  {\it Engine Type} appears only for an engine-sensor BLE device
  (ignition scaling); heart-rate bands do not show it.
  A Xiaomi Smart Band talks GATT on a random LE address; XCSoar
  follows that live device.  Enable {\it Heart rate broadcast} and
  leave the Xiaomi app (and a second XCSoar copy) disconnected so the
  standard heart-rate service is free.
  A pulse oximeter is selected as {\it BLE sensor} as well.  The
  Blood Oxygen InfoBox shows its saturation.
  The Devices list shows {\it Heart Rate}, {\it Blood Oxygen},
  {\it Pressure}, {\it Temperature}, and {\it Relative humidity}
  among the other sensor flags, and {\it Battery=N\%} when remaining
  charge is known, including BLE sensors that expose the standard
  heart-rate, Pulse Oximeter, Environmental Sensing, and Battery
  services.
