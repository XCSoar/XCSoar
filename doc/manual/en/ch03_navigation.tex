...
\begin{enumerate}
\item Enable pan mode by pressing (or by gesture)  \gesture{Up - Right - Down - Left}
\begin{quote}
...
menu:
\begin{quote}
\bmenu{Display ..}\blink\bmenu{Terrain On} \\
\bmenu{Display ..}\blink\bmenu{Topo. On}
\end{quote}
...
\begin{quote}
\bmenu{Display ..}\blink\bmenu{Labels None}
\end{quote}

Other options for display decluttering include:

\jindent{\bmenut{Labels}{Task \& Landables}}{  Shows labels for the waypoints in  the active task and any landable fields (based on the waypoint attributes in the waypoints file).  Other waypoints are shown bu not labeled. }
...
Note that in all cases, the label format is configurable in the "Waypoint Display" configuration menu.  \config{labels}
...
menu:
\begin{quote}
\bmenu{Display ..}\blink\bmenu{Trail}
\end{quote}
...
to the end of the flight. This thermal history is accesible through the map 
...
%{\it DIAGRAM CUTOUT SHOWING GLIDE RANGE FOOTPRINT.  NO TOPOGRAPHY,
...
\section{Flight status}\label{sec:flight-status}

The status dialog is a multi-tabular dialog giving overview information on the 
flight, system, task, rules and times.
It is accessed via the gesture "S", or with the menu under: \gesture{Left - Down - Right - Down - Left}
\begin{quote}
...
\subsection*{Position}
The flight status dialog shows the status of the aircraft's locality, and can be useful when giving position reports. It shows the location of the aircraft, the
...
\sketch{figures/status-times.png}s
