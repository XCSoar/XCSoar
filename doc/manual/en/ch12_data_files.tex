...
Terrain and topography files are accessed continuously while XCSoar is
running, so they should be located on reasonably fast storage.
Modern memory cards usually meet the required performance.

All data files should be copied into the directory
\verb|XCSoarData|.  See Chapter~\ref{cha:installation} for where that
...
\section{Airfields or Waypoint details}\label{sec:airfield-details}

The airfields or waypoint details file (extension \verb|.txt|) is a simple text
...
\item Speed in km/h for third measurement point, (usually a high cruise speed).
...
\item The max.\ cruise speed in m/s.  XCSoar uses this as an upper limit
...
converters at \url{https://xcsoar.readthedocs.io/en/latest/task_file.html}.
...
The flight list estimates takeoff and landing times from recorded fixes.
For IGC files with unknown aircraft performance, sustained climb and height
gain can indicate a slow launch.
Sustained climbs can also prevent a premature landing estimate at low speed.
