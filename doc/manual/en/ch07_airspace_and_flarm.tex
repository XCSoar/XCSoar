...
This brings up a dialogue with a list of map elements in the near of this 
location. All SUAs are listed with similar details as is
provided when an actual warning is raised.  Selecting the SUA on the list and 
pressing \button{Details} or the enter key displays all airspace details. 
\sketch{figures/airspace_mapelements.png}
 The query returns all airspace regions when overlapping airspace 
is visible at the query location.
...
\subsection*{Airspace filter dialogue}\label{sec:airsp-filt-dial}
...
Airspace}.
\item From the configuration dialogue, menu under
\bmenu{Config}\blink\bmenu{Config}\blink\bmenu{Setup System} then in the
Airspace page, press the \button{Filter} button.
\end{itemize}
