...
drawn.  Fill padding is drawn inside the airspace boundary, keeping
overlapping borders visible.

A small label on each airspace shows its ceiling and base.  Labels
can be switched on or off with Label visibility in the configuration
settings.  See Section~\ref{sec:map-airspace}.  When two labels would
cover each other they are moved so both remain readable.
...
The FLARM targets on the map are drawn as symbols that indicate the
direction the FLARM target is heading, as well as the collision
risk\config{flarm-on-map}.  By default the symbol shows the aircraft
type reported by FLARM, ADS-B or OGN: a glider, a powered aircraft, a
helicopter, a hang glider, a paraglider, a balloon and so on.  Targets
of unknown type get a generic chevron symbol.  The classic arrow head
for all targets can be selected instead\config{traffic-symbol}.
Note that these symbols are oriented according to the display
orientation.  For example, if the orientation is track-up, then the
symbols show the relative track bearing of the target to the aircraft.
If the orientation is north-up, then the symbols show the absolute
track bearing of the target.  In the map item list opened with
`What's here?' the symbols are always drawn north-up, so the course of
each target can be read directly from its symbol.
...
with one or two red circles around the traffic symbol.  The display corner used
