...
On Linux, choosing Quit also offers controls for the system actions which are
available without authentication, such as restarting or powering off the
device.  These controls are also available on Kobo.  Cancel is the default
choice.  After choosing an action, XCSoar saves its state and closes its devices
before asking the operating system to perform the action.  Other platforms
continue to show only the normal confirmation for quitting XCSoar.
...
For a 320x240 display in portrait mode, there are four InfoBoxes above
...
Press and hold an InfoBox past a tap; it fills from the bottom.
Lift while it is full to enter the arrange mode with that InfoBox
selected, or slide off to cancel.  The `\emph{Arrange InfoBoxes}'
item of the Quick Menu opens the same mode without picking a box up.
The screen uses the same backdrop as the InfoBox set dialog, with the
map still showing through, and every InfoBox is replaced by a card
showing only its number and its caption.

A tap on a card explains what that InfoBox shows.  Drag it onto
another one to exchange the two; the InfoBox which makes way slides
over into the slot the dragged one has left.  Hold still and the card fills from the bottom; lift to open the
list of all available InfoBoxes, or slide off to cancel.  Once the
card follows the finger, holding no longer opens the list.  A second
finger on the screen aborts the drag.

Without a touch-screen the cursor keys and a remote stick select an
InfoBox.  Enter takes the selected InfoBox, the cursor keys then move
it, and Enter puts it down again.  Taking an InfoBox and putting it
down without having moved it opens the list of available InfoBoxes
instead.  Tab leaves the cards and reaches the Help and Close
buttons; so does a cursor step toward them (down on a portrait page,
left on a landscape one).
...
The InfoBox sets are composed in the same view; the dialog buttons
...
Message dialogues adjust their text and controls when the application window
or display orientation changes.
...
\item[Data Management] Provides file downloads, import, backup and flight
  ...
  A line under the list shows how many IGC files have a detected launch and
  landing, and their total time as hours:minutes. It keeps updating while
  files are still being scanned.
  WeGlide Upload opens that
  list with only IGC files. The Upload Flight button sends the
  highlighted flight.
\end{description}
...
checklist.  Tapping the text of a checkbox line toggles it.  A link
on that line, such as a VHF frequency, still opens the link.
Checkbox ticks stay until you leave XCSoar or choose a
different checklist file. Editing a page's text clears its saved ticks.
...
While you draw, XCSoar shows the path as a red line with a soft
shadow.  The line is slightly transparent while the figure drawn so
far is not a known gesture.  Once it is, a label at the top of the map names
what the gesture does, for example ``Zoom in'' or ``Analysis''; it
changes as you continue drawing, and what it names happens when you
lift the finger.  With haptic feedback enabled, a light tick tells you
that the figure has turned into another known gesture, for example
from ``Analysis'' to ``Pan''; lifting the finger to run the gesture
gives a stronger one.
...
finger positions (Android, iOS, Linux Wayland), pinching zooms the
map about the point between the fingers, and twisting rotates it
temporarily.  A two-finger
