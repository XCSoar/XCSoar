...
You should not start using XCSoar without knowing about the `Alternates' feature. Any `Task' related item in the navigation menu are used for planned cross country flight and certainly the second step.
...
The configuration menu is typically part of the ground interaction with XCSoar. You are not expected to spend much time in-flight with tweaking the configuration, except you manually adjust wind or MacCready settings. 
...
The information menu is always a good address, when not only a clue on how to set MacCread is requested, but rather more elaborate help on a larger scope tactical decision on your flight is requested.
...
Vario 1/2 & \bmenut{Airframe}{Switches} & \bmenut{Setup}{Audio}
 & \bmenut{Manual}{Demo} & \bmenut{Setup}{Stall} & \bmenus{Accel} \\ \\
Vario 2/2 & \bmenut{ASI}{Zero} & \bmenut{Accel}{Zero}
...
XCSoar allows the pilot to define various sets of InfoBoxes that are appropriate to various stages of flight (e.g. when circling in a thermal, flying between thermals, on final glide, etc.). XCSoar can be configured to automatically switch from one InfoBox display to another based on the mode of flight, or you can manually scroll through the various InfoBox displays, including a display with a full-screen map and no InfoBoxes at all.

To toggle through the various InfoBox displays, using a touch-screen:
\gesture{Right} 
or
\gesture{Left}
...
InfoBox with the touch-screen or mouse.  This brings up a set of InfoBoxes:
...
\item[\bmenu{Setup}]
	Allows you to change the behaviour of the setting related to the InfoBox (for example, changing from Auto to Manual MacCready setting mode) or choose to setup of the InfoBox itself (that is, choosing \bmenu{Setup InfoBox} will display a list dialogue of all available InfoBoxes from which to choose.
...
InfoBoxes can either be changed by calling the configuration dialogue from the
menu \bmenu{Config ..}\blink\bmenu{Setup System} Screen 21, "InfoBox Modes" or by
performing a long press on the InfoBox that should be changed. In the second case a list dialogue opens,
...
\item[Checklist] A multi-page custom check-list
...
These check-lists may include: daily inspection, preflight, outlanding,
...
you drag your finger down and afterwards to the right over the screen, \gesture{down-right} the gesture "DR" is detected,
