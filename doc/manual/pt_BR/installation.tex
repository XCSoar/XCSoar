...
Se é um piloto com urgência, os autores sugerem que você utilize o Manual XCSoar-in-a-Flash através da checklist de passo-a-passo.  O manual resumido está disponível em: 
 \xcsoarwebsite{/discover/manual.html}.
...
O XCSoar é compatível com qualquer GPS que emita dados NMEA.  A maioria dos dispositivos modernos de Android e iOS tem um receptor de GPS interno.  Muitos destes telefones e tablets também têm um sensor de pressão barométrica; o XCSoar o usa para altitude quando nenhum altímetro externo melhor está conectado.  No Android, o XCSoar também usa o acelerômetro do telefone como sensor de G (fator de carga).  Por várias razões, é aconselhável conectar um ou mais dispositivos externos:
...
Para instalar o XCSoar, conecte o Kobo ao seu computador via USB. O Kobo mostra como um dispositivo de armazenamento no seu PC; abre e cria diretório chamado \texttt{.kobo} (note o ponto final antes do nome do diretório).  Baixe o arquivo \texttt{KoboRoot.tgz} do site do XCSoar neste diretório (\xcsoarwebsite{/hardware/}). Desconecte o Kobo e reinicie-o (desligue completamente e religue).  Você verá a mensagem “Updating” e após alguns minutos, o Kobo mostra um menu que permite você carregar o XCSoar ou o software de leitura de e-book da Kobo.
...
Instale o XCSoar na Apple App Store.  Os arquivos de dados ficam em
\texttt{Documents/XCSoarData} e podem ser copiados com o aplicativo Arquivos.

\subsection*{No macOS}

Instale o XCSoar na Apple App Store, ou baixe o pacote macOS no site do
XCSoar.  Os arquivos de dados ficam em \texttt{XCSoarData} na pasta pessoal.

\subsection*{Para a Windows PC}

Duas versões Windows estão disponíveis:
\begin{itemize}
\item \textbf{64 bits} (alvo ``WIN64OPENGL''): a versão recomendada.
  Execute o instalador, ou use o arquivo \verb|.zip| para uma instalação
  manual.
\item \textbf{32 bits} (alvo ``WIN32OPENGL''): o mesmo para Windows
  32 bits.
\end{itemize}
...
Documents}'')
\item[macOS]
\texttt{XCSoarData} na pasta pessoal.
\item[iOS]
\texttt{XCSoarData} na pasta Documents do aplicativo, acessível pelo
aplicativo Arquivos.
\item[Unix/Linux]
...
O XCSoar irá gerar um número adicional de arquivos enquanto roda.  Estes serão alocados no diretório  \texttt{XCSoarData} (Windows, macOS, iOS e
dispositivos Android), ou no diretório \texttt{.xcsoar} para (Unix/Linux
