...
As provas ‘Ir Para’ podem ser definidas selecionando um waypoint no mapa, pela lista de waypoints ou outro mecanismo, como por exemplo uma caixa de diálogo, e selecione  \bmenuw{Goto}. No modo de prova, selecione
\bmenug{Nav 2} e então \blink\bmenug{Task Resume} e a prova será retomada (se houver alguma).
...
, selecione e\bmenuw{select} para selecionar o waypoint e mostrar os detalhes.
...
\bmenuw{GoTo} para iniciar imediatamente a navegação a este waypoint.  Este botão cancela a prova atual.
...
A primeira página é uma calculadora.  Mostra vários cálculos relativos à prova ativa, como descrito em detalhes abaixo:  Há botões para acionar os diálogos \bmenuw{Turn Points}, \bmenuw{Manage}, 
e \bmenuw{Rules}, bem como o botão \bmenuw{Close} .
...
Fechando a janela, o valor digitado de MacCready será usado como ajuste. Se o botão \bmenuw{Cancel} for pressionado, o ajuste de MacCready não será afetado.

The \bmenuw{Pilões} button, para provas AAT tasks, ajuste a faixa (aumente ou diminua) para que o tempo estimado exceda o tempo designado para prova subtraído de 5 minutos. A faixa é ajustada largamente, geralmente todos os alvos são configurados para ''auto", significando que o piloto não tem que ajustar manualmente a faixa para achar o caminho para a chegada no tempo designado, diminuindo o trabalho do piloto.
...
\item [\bmenuw{Salvar}] SSalva a prova atual.  Clicando em 
  \bmenuw{Save} o piloto será questionado a entrar com um nome de arquivo para ser salvo.
\end{itemize}
...
A janela de \bmenuw{Pilões} dialogue brings up the list of waypoints in the task. 
...
FPara os pontos de provas que necessitam serem armados, \bmenug{Próx. Pto. Virada} se torna 
...
\blink\bmenuw{Pilões} select start point, \menulabelr{\bmenug{Nav 1/2}\blink
\bmenug{Prova}}\bmenuw{Editar Ponto}\blink\bmenuw{Habilitar Starts Alternados}

Na página de ponto de start do Gerenciador de Tarefas, mude para \bmenuw{Habilitar Starts Alternados}. Outra tela aparecerá para definir o ponto alternativo.  Neste caso, os pontos de start devem ter sidos definidos previamente.  Depois de selecionado \bmenuw{Habilitar Starts Alternados} e teclado \bmenuw{Edit Alternates}. After selecting ``Add Alternate Start'' and touching 
\bmenuw{Realocar} abrirá a janela para seleção de waypoints.
 
Tendo configurado alguns Starts alternativos, o método “próximo waypoint” será novamente aumentado.  Antes da detecção de um start vário e tendo armado o start manualmente, no menu de botões irá aparecer  \bmenug{Próximo Start}.

Resumindo, todos os rótulos dinâmicos em \bmenug{Nav 1/2} mostrará os comandos a serem executados para selecionar os waypoints e condições, tanto na ordem normal como inversa.
...
\bmenug{Alternativos}} Six landing 
options are maintained. .  Seis opções de pousos serão mantidas.  São filtrados pelos critérios de configuração (simples, prova ou casa). \config{alternatesmode} 
...
Igualmente quando se tem toda a lista de waypoints, as informações adicionais podem ser vistas clicando em “Detalhes” antes de decidir para qual ponto se navegara.  Selecione um waypoint da lista e clique em \bmenuw{Ir Para}.
