...
des appareils Android et iOS intègrent un GPS. Beaucoup de ces téléphones et
tablettes ont aussi un capteur de pression barométrique~; XCSoar l'utilise pour
l'altitude lorsqu'aucun meilleur altimètre externe n'est connecté. Sous Android,
XCSoar utilise aussi l'accéléromètre du téléphone comme capteur G (facteur de
charge). Pour diverses
raisons, il est néanmoins désirable de se connecter à un ou plusieurs appareils
externes~:
\begin{itemize}
...
Installez XCSoar depuis l'App Store d'Apple. Les fichiers de données sont
dans \texttt{Documents/XCSoarData} et peuvent être copiés avec l'application
Fichiers.

\subsection*{Sous macOS}

Installez XCSoar depuis l'App Store d'Apple, ou téléchargez le paquet macOS
depuis le site XCSoar. Les fichiers de données vont dans \texttt{XCSoarData}
dans votre répertoire personnel.

\subsection*{Sur un PC sous Windows}

Deux versions Windows sont disponibles~:
\begin{itemize}
\item \textbf{64 bits} (cible ``WIN64OPENGL'')~: version recommandée.
  Installez avec l'installateur, ou utilisez le fichier \verb|.zip|
  pour une installation manuelle.
\item \textbf{32 bits} (cible ``WIN32OPENGL'')~: identique pour Windows
  32 bits.
\end{itemize}
...
Documents}'')
\item[macOS]
\texttt{XCSoarData} est dans votre répertoire personnel.
\item[iOS]
\texttt{XCSoarData} est dans le répertoire Documents de l'application,
accessible via l'application Fichiers.
\item[Unix/Linux]
...
seront placés dans le répertoire \texttt{XCSoarData} (PC sous Windows, macOS,
iOS et appareils Android), ou dans le répertoire \texttt{.xcsoar} (PC sous
