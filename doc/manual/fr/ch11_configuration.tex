...
\end{itemize}
La description de tout ceci en détail va au delà du contenu d'un manuel utilisateur. Pour plus d'information l'utilisateur peut parcourir le wiki d'XCSoar. \url{https://github.com/XCSoar/xcsoar/issues/wiki}
...
%%%%%%%%%%%%%%%%%%
\section{Apparence / Pages InfoBoxes}\label{conf:screenpages}
...
Lors de l'édition d'une page, les options suivantes sont disponibles pour
les pages carte~:
\begin{description}
\item[Zone principale]
  Contenu principal de la page, par exemple \emph{Carte} ou \emph{Carte
  (nord en haut)}.
\item[InfoBoxes]
  Quel ensemble d'InfoBoxes est affiché à côté de la zone principale.
\item[Zone inférieure]
  Panneau optionnel sous la zone principale.  Choisissez \emph{Commandes
  météo} pour afficher les contrôles RASP ou EDL en vol lorsque la page
  utilise une surcouche météo.
\item[Surcouche carte]
  Surcouche météo optionnelle sur les pages carte~: \emph{Aucune},
  \emph{RASP} ou \emph{EDL}.  RASP nécessite un fichier RASP chargé ; EDL
  nécessite que \emph{Page météo EDL} soit activée dans la configuration
  météo du système.
\item[Couche RASP]
  Lorsque \emph{Surcouche carte} est \emph{RASP}, sélectionne le champ
  RASP affiché sur cette page.
\end{description}
...
Pour l'instant il y a deux protocoles de tracking disponibles. {\it 'Sky-Lines'} comme projet interne à XCSoar : pour plus de détails sur ce service aller regarder le site {\href{https://skylines.aero}{skylines.aero}}.\\
...
\item[Key] Vous devez créer votre propre clé (key)  sur le site web {\href{https://skylines.aero/tracking/info}{skylines.aero/tracking/info}} et la saisir dans ce champ afin d'être identifié par le service de tracking.
