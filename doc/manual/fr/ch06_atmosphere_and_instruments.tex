...
Les commandes EDL sont disponibles dans la boîte de dialogue
\menulabelr{\bmenug{Info}\blink\bmenug{Météo}} sur l'onglet
\bmenuw{EDL}. Cet onglet permet au pilote de~:
\begin{description}
\item[Prévision]
...
\item[Toujours afficher EDL sur la carte]
  Conserver la surcouche EDL visible sur les pages de carte ordinaires.
  Lorsque cette option est désactivée, la surcouche n'est affichée que
  sur la page EDL dédiée.
\item[Jour en cache]
...
\item[Précharger le jour]
  ...
  actuellement sélectionné. Cette fonction est destinée à la préparation
  du vol, afin que les données EDL restent disponibles si la connexion
  Internet est perdue en vol.
...
Les surcouches EDL peuvent être utilisées de deux manières~:
\begin{description}
\item[Page EDL dédiée]
  Dans la boîte de dialogue de configuration des pages
  (Section~\ref{conf:screenpages}), sélectionnez \emph{Météo EDL}
  comme \emph{Zone principale} et \emph{EDL} comme \emph{Zone
  inférieure}. En entrant sur cette page, l'affichage revient à l'heure
  UTC courante et sélectionne le niveau EDL disponible le plus proche de
  l'altitude actuelle du planeur lorsque cette altitude est fournie par
  les capteurs connectés. La légende de surcouche en haut à droite
  affiche l'heure locale active, le niveau de pression sélectionné et
  son altitude approximative.
\item[Surcouche sur la carte normale]
  Ouvrez \menulabelr{\bmenug{Info}\blink\bmenug{Météo}}
  \bmenuw{EDL}, sélectionnez l'heure de prévision et le niveau
  souhaités, puis activez \bmenuw{Toujours afficher EDL sur la carte}.
  La carte mobile normale reste utilisée, avec l'image météo EDL
  affichée en surcouche au-dessus de la carte.
\end{description}
...
Au cours des vérifications pré-vol et pendant le vol, vous pouvez vérifier les prévisions météo et les données météo en temps réel dans le menu \menulabelr{\bmenug{Info}\blink\bmenug{Météo}} \bmenug{Info}.
