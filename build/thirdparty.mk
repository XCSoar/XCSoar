...
ifeq ($(ANDROID_LEGACY),y)
  THIRDPARTY_OUTPUT_DIR = $(TARGET_OUTPUT_DIR)/lib_legacy
else
  THIRDPARTY_OUTPUT_DIR = $(TARGET_OUTPUT_DIR)/lib
endif
THIRDPARTY_LIBS_DIR = $(THIRDPARTY_OUTPUT_DIR)/$(ACTUAL_HOST_TRIPLET)
...
$(THIRDPARTY_LIBS_DIR)/stamp:
	./build/thirdparty.py $(THIRDPARTY_OUTPUT_DIR) $(TARGET) $(HOST_TRIPLET) $(ACTUAL_HOST_TRIPLET) "$(TARGET_ARCH)" "$(TARGET_CPPFLAGS)" "$(filter-out $(THIRDPARTY_LDFLAGS_FILTER_OUT),$(TARGET_LDFLAGS))" $(CC) $(CXX) $(AR) "$(ARFLAGS)" $(RANLIB) $(STRIP)
	touch $@
