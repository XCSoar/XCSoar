...
$(APP_BUNDLE): $(TARGET_BIN_DIR)/xcsoar Data/OSX/Info.plist.in.xml $(OSX_LOGO) $(ANGLE_FETCH_STAMP)
	@$(NQ)echo "  APP     $@"
...
ifeq ($(USE_ANGLE),y)
	...
	$(Q)install_name_tool -change "$$(otool -L $(APP_FRAMEWORKS)/libEGL.dylib | grep libGLESv2 | awk '{print $$1}')" "@rpath/libGLESv2.dylib" $(APP_FRAMEWORKS)/libEGL.dylib 2>/dev/null || true
	$(Q)install_name_tool -change "./libEGL.dylib" "@rpath/libEGL.dylib" $(APP_MACOS)/xcsoar 2>/dev/null || true
	$(Q)install_name_tool -change "./libGLESv2.dylib" "@rpath/libGLESv2.dylib" $(APP_MACOS)/xcsoar 2>/dev/null || true
endif
# Bundle Homebrew libraries and fix all @rpath references
	@$(NQ)echo "  BUNDLE  Homebrew libraries"
	$(Q)install_name_tool -add_rpath "@executable_path/../Frameworks" $(APP_MACOS)/xcsoar 2>/dev/null || true
	$(Q)for lib in $$(otool -L $(APP_MACOS)/xcsoar | grep -E '/opt/homebrew|/usr/local|$(ANGLE_PREFIX)' | awk '{print $$1}'); do \
		[ -f "$$lib" ] || continue; \
		base=$$(basename "$$lib"); \
		[ -f "$(APP_FRAMEWORKS)/$$base" ] && continue; \
		cp "$$lib" $(APP_FRAMEWORKS)/ && chmod u+w $(APP_FRAMEWORKS)/$$(basename "$$lib"); \
		install_name_tool -id "@rpath/$$base" $(APP_FRAMEWORKS)/$$base; \
		install_name_tool -change "$$lib" "@rpath/$$base" $(APP_MACOS)/xcsoar; \
	done
	...
	while true; do \
		...
		for bundled in $(APP_FRAMEWORKS)/*.dylib; do \
			...
			for lib in $$(otool -L "$$bundled" | grep -E '/opt/homebrew|/usr/local' | awk '{print $$1}'); do \
				...
				[ -f "$(APP_FRAMEWORKS)/$$base" ] && continue; \
				cp "$$lib" $(APP_FRAMEWORKS)/ && chmod u+w $(APP_FRAMEWORKS)/$$base; \
				install_name_tool -id "@rpath/$$base" $(APP_FRAMEWORKS)/$$base; \
				install_name_tool -change "$$lib" "@rpath/$$base" "$$bundled"; \
...
# Fix references between bundled libraries
	$(Q)for bundled in $(APP_FRAMEWORKS)/*.dylib; do \
		...
		for lib in $$(otool -L "$$bundled" | grep -E '/opt/homebrew|/usr/local' | awk '{print $$1}'); do \
			base=$$(basename "$$lib"); \
			[ -f "$(APP_FRAMEWORKS)/$$base" ] && install_name_tool -change "$$lib" "@rpath/$$base" "$$bundled" 2>/dev/null || true; \
		done; \
	done
