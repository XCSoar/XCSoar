...
class AngleProject(Project):
    ...
    def _install_marker(self, toolchain: AnyToolchain) -> str:
        return os.path.join(
            toolchain.install_prefix,
            f".angle-{self.RELEASE_REVISION}.installed",
        )
    ...
    def is_installed(self, toolchain: AnyToolchain) -> bool:
        ...
        return os.path.isfile(self._install_marker(toolchain)) and all(
            os.path.isfile(path) for path in required
        )
    ...
    def _generate_import_library(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            gendef_args = [gendef]
            if toolchain.host_triplet.startswith("i686-"):
                # ANGLE's 32-bit Windows exports need stdcall decoration in the
                # generated import library, otherwise glCreateProgram and
                # similar symbols stay undecorated and the Win32 link step
                # fails.
                gendef_args.append("--assume-stdcall")

            subprocess.check_call(gendef_args + [dll_path], cwd=tmp, timeout=60)
    ...
    def _build(self, toolchain: AnyToolchain) -> None:
        marker = self._install_marker(toolchain)
        try:
            os.unlink(marker)
