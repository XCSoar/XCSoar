...
public:
  ...
  {
    return _("Forecast hour");
  }

  unsigned GetMinimumValueWidth() const noexcept override
  {
    return UIGlobals::GetDialogLook().text_font.TextSize("00:00").width +
...
public:
  ...
  unsigned GetMinimumValueWidth() const noexcept override
  {
    return UIGlobals::GetDialogLook().text_font.TextSize("900 hPa").width +
      2 * Layout::GetTextPadding();
  }
...
EdlControlsWidget::GetMinimumSize() const noexcept
{
  const unsigned caption_width =
    UIGlobals::GetDialogLook().text_font.TextSize("Forecast hour").width;
...
EdlControlsWidget::Show(const PixelRect &rc) noexcept
{
  ...
  if (dedicated_page && EDL::EnterDedicatedPage()) {
