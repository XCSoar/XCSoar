...
{
  // Cache the result string to ensure the returned pointer remains valid.
  static std::string cached_result;
  // Cache file sizes to avoid repeated filesystem stat calls on each repaint.
  static std::unordered_map<std::string, uint64_t> size_cache;

  const std::string path_utf8 = it.path.ToUTF8();
  uint64_t sz = 0;
  auto found = size_cache.find(path_utf8);
  if (found != size_cache.end())
    sz = found->second;
  else {
    sz = File::GetSize(it.path);
    size_cache.emplace(path_utf8, sz);
  }

  char buf[32];
  FormatByteSize(buf, sizeof(buf), sz, false);
  cached_result = buf;
  return cached_result.c_str();
}
...
ShowExportFlightsDialog()
{
  ...
  dialog.AddButton(_("WeGlide Upload"), [file_widget]() { PerformWeGlideUpload(file_widget); });
