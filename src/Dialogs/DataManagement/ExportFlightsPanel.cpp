...
PerformExport(FileMultiSelectWidget *file_widget)
{
  ...
  const AllocatedPath &target = StorageLocationPickerDialog::GetLastTarget();
  if (target == nullptr) {
  ...
  ExportJob job(selected, Path(StorageLocationPickerDialog::GetLastTarget()), completed, skipped, failed);
