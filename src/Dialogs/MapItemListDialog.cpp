...
MapItemListWidget::OnGotoClicked()
{
  ...
  if (item.type == MapItem::Type::LOCATION) {
    const auto &loc_item = (const LocationMapItem &)item;
    ...
    // Create temporary goto waypoint
    ScopeSuspendAllThreads suspend;
    auto &way_points = *data_components->waypoints;
    way_points.AddTempPoint(location, elevation, _T("(goto)"));
    way_points.Optimise();
    
    // Lookup the waypoint we just created
    waypoint = way_points.LookupName(_T("(goto)"));
    if (!waypoint)
      return;
  } else {
    waypoint = ((const WaypointMapItem &)item).waypoint;
  }
                  ...
                  ProtectedAirspaceWarningManager *airspace_warnings)
{
  ...
  case MapItem::Type::LOCATION: {
    ...
    // Create temporary goto waypoint for display
    ScopeSuspendAllThreads suspend;
    waypoints->AddTempPoint(location, elevation, _T("(goto)"));
    waypoints->Optimise();
    
    // Lookup the waypoint we just created and show details
    auto wp = waypoints->LookupName(_T("(goto)"));
    if (wp)
