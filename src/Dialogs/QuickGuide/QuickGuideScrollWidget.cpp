...
bool
QuickGuideScrollWidget::KeyPress(unsigned key_code) noexcept
{
  // Let the child widget handle the key first (for link/checkbox navigation)
  if (widget->KeyPress(key_code))
    return true;

  // Handle Up/Down for scrolling if child didn't handle it
  switch (key_code) {
  case KEY_UP:
    GetWindow().ScrollBy(-GetWindow().GetScrollStep());
    return true;
