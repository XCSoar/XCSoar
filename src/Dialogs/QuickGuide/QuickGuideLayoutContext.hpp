...
public:
   ...
   */
  unsigned DrawTextBlock(const Font &font, int left, const char *text,
   ...
   */
  unsigned DrawTextBlock(const char *text) noexcept {
  ...
  unsigned DrawLinkLine(LinkAction link, const char *text) noexcept {
   ...
   */
  template<typename LinkAction>
  void DrawNumberedItem(int number, const char *description,
                        LinkAction link, const char *link_text) noexcept {
    DrawNumber(number);
   ...
   */
  template<typename LinkAction>
  void DrawCheckboxItem(bool checked, const char *description,
                        LinkAction link, const char *link_text) noexcept {
  ...
  /**
   * Get the final content height after layout (relative to rc.top).
   */
  [[gnu::pure]]
  unsigned GetHeight() const noexcept {
    return static_cast<unsigned>(y - rc.top);
  }
};
