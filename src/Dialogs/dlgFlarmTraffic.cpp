...
static FlarmTrafficControl2 *wdf;
                                    ...
                                    fixed climb_rate) const
{
  ...
  // Calculate positions
  PixelScalar max_height = std::max(unit_height, value_height);
  PixelScalar y = rc.top + look.info_units_font.GetHeight() + max_height;

  // Paint value
  canvas.DrawText(rc.right - unit_width - space_width - value_width, y - value_height, buffer);
                                    ...
                                    fixed distance) const
{
  ...
  // Paint label
  canvas.Select(look.info_labels_font);
  canvas.DrawText(rc.left, rc.bottom - max_height - look.info_labels_font.GetHeight(), _("Distance"));
}
                                           ...
                                           fixed relative_altitude) const
{
  ...
  // Paint value
  canvas.DrawText(rc.right - unit_width - space_width - value_width, rc.bottom - value_height, buffer);
  ...
  canvas.DrawText(rc.right - label_width, rc.bottom - max_height - look.info_labels_font.GetHeight(), _("Rel. Alt."));
}
                            ...
                            const WindowStyle style)
{
  ...
  wdf = new FlarmTrafficControl2(look.flarm_dialog);
