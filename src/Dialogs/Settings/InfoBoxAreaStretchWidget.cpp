...
#include "InfoBoxAreaStretchWidget.hpp"
...
struct Edge {
  DisplaySettings::InfoBoxAreaStretchEdge bit;
};

constexpr Edge edges_table[] = {
  { DisplaySettings::INFOBOX_AREA_STRETCH_TOP },
  { DisplaySettings::INFOBOX_AREA_STRETCH_RIGHT },
  { DisplaySettings::INFOBOX_AREA_STRETCH_BOTTOM },
  { DisplaySettings::INFOBOX_AREA_STRETCH_LEFT },
};
 ...
 * stands for the InfoBox area: on a stretched edge it reaches the screen
 * border, on the others it keeps a margin for the system UI.
 */
class InfoBoxAreaStretchWindow final : public PaintWindow {
...
public:
  InfoBoxAreaStretchWindow(const DialogLook &_look, uint8_t &_edges) noexcept
...
private:
  ...
  /**
   * The InfoBox area as it results from the current selection.
   */
  [[gnu::pure]]
  PixelRect GetSafeRect(const PixelRect &screen) const noexcept {
    ...
    if ((edges & DisplaySettings::INFOBOX_AREA_STRETCH_TOP) == 0)
      rc.top += band;
    if ((edges & DisplaySettings::INFOBOX_AREA_STRETCH_BOTTOM) == 0)
      rc.bottom -= band;
    if ((edges & DisplaySettings::INFOBOX_AREA_STRETCH_LEFT) == 0)
      rc.left += band;
    if ((edges & DisplaySettings::INFOBOX_AREA_STRETCH_RIGHT) == 0)
      rc.right -= band;
  ...
  /**
   * An arrow pointing out of the InfoBox area on a stretched edge, and
...
void
InfoBoxAreaStretchWindow::DrawMap(Canvas &canvas, const PixelRect &screen,
                               const PixelRect &safe) const noexcept
{
...
void
InfoBoxAreaStretchWindow::DrawAircraft(Canvas &canvas, PixelPoint centre,
...
void
InfoBoxAreaStretchWindow::DrawArrow(Canvas &canvas, const PixelRect &screen,
                                 unsigned i) const noexcept
{
...
void
InfoBoxAreaStretchWindow::OnPaint(Canvas &canvas) noexcept
{
  ...
  /* the InfoBox area is outlined on the map rather than covering it, so
...
bool
InfoBoxAreaStretchWindow::OnMouseDown(PixelPoint p) noexcept
{
  const PixelRect screen = GetScreenRect();
...
bool
InfoBoxAreaStretchWindow::OnKeyCheck(unsigned key_code) const noexcept
{
  switch (key_code) {
  case KEY_UP:
    return cursor != TOP;
...
bool
InfoBoxAreaStretchWindow::OnKeyDown(unsigned key_code) noexcept
{
...
PixelSize
InfoBoxAreaStretchWidget::GetMinimumSize() const noexcept
{
...
PixelSize
InfoBoxAreaStretchWidget::GetMaximumSize() const noexcept
{
...
void
InfoBoxAreaStretchWidget::Prepare(ContainerWindow &parent,
                               const PixelRect &rc) noexcept
{
  ...
  auto window = std::make_unique<InfoBoxAreaStretchWindow>(look, edges);
