...
public:
  SiteConfigPanel() : RowFormWidget(UIGlobals::GetDialogLook()) {}
                         ...
                         [[maybe_unused]] const PixelRect &rc) noexcept
{
  ...
  AddFile(_("Map database"),
          _("The name of the file (.xcm) containing terrain, topography, and "
            "optionally "
            "waypoints, their details and airspaces."),
          ProfileKeys::MapFile, _T("*.xcm\0*.lkm\0"), FileType::MAP);

  AddFile(_("Waypoints"),
          _("Primary waypoints file.  Supported file types are "
  ...
  AddFile(_("More waypoints"),
          _("Secondary waypoints file.  This may be used to add waypoints for "
            "a competition."),
  ...
  AddFile(_("Watched waypoints"),
          _("Waypoint file containing special waypoints for which additional "
            "computations like "
            "calculation of arrival height in map display always takes place. "
            "Useful for "
            "waypoints like known reliable thermal sources (e.g. powerplants) "
            "or mountain passes."),
  ...
  AddFile(_("Waypoint details"),
          ...
          ProfileKeys::AirfieldFile, _T("*.txt\0"), FileType::WAYPOINTDETAILS);
  SetExpertRow(AirfieldFile);

  AddFile(_("FLARM Device Database"),
          _("The name of the file containing information about registered "
            "FLARM devices."),
          ProfileKeys::FlarmFile, _T("*.fln\0"), FileType::FLARMNET);

  AddFile(_T("RASP"), nullptr, ProfileKeys::RaspFile, _T("*-rasp*.dat\0"),
          FileType::RASP);
}
...
SiteConfigPanel::Save(bool &_changed) noexcept
{
  ...
  WaypointFileChanged |=
      SaveValueFileReader(WaypointFile, ProfileKeys::WaypointFile);
  WaypointFileChanged |= SaveValueFileReader(
      AdditionalWaypointFile, ProfileKeys::AdditionalWaypointFile);
  WaypointFileChanged |= SaveValueFileReader(WatchedWaypointFile,
                                             ProfileKeys::WatchedWaypointFile);

  AirspaceFileChanged =
      SaveValueFileReader(AirspaceFile, ProfileKeys::AirspaceFile);
  AirspaceFileChanged |= SaveValueFileReader(
      AdditionalAirspaceFile, ProfileKeys::AdditionalAirspaceFile);

  FlarmFileChanged = SaveValueFileReader(FlarmFile, ProfileKeys::FlarmFile);

  AirfieldFileChanged =
      SaveValueFileReader(AirfieldFile, ProfileKeys::AirfieldFile);
  ...
  changed = WaypointFileChanged || AirfieldFileChanged || MapFileChanged ||
            FlarmFileChanged || RaspFileChanged;
