...
static void UpdateWind(bool set) {
  ...
  if ((ws != XCSoarInterface::Basic().aircraft.wind.norm)
      ||(wb != XCSoarInterface::Basic().aircraft.wind.bearing)) {
    /* JMW illegal
    if (set) {
...
static void OnWindSpeedData(DataField *Sender, DataField::DataAccessKind_t Mode){
  ...
  switch(Mode){
    case DataField::daGet:
      ...
      Sender->Set((double)(SPEEDMODIFY * XCSoarInterface::Basic().aircraft.wind.norm));
    break;
...
static void OnWindDirectionData(DataField *Sender, DataField::DataAccessKind_t Mode){
  ...
  switch(Mode){
    case DataField::daGet:
      lastWind = XCSoarInterface::Basic().aircraft.wind.bearing;
      if (lastWind < 0.5)
        lastWind = 360.0;
      Sender->Set(lastWind);
    break;
