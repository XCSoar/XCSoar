...
MainWindow::LayoutMapArea() noexcept
{
  ...
  /* the part of the map that nothing else covers */
  PixelRect visible_rect = widget_rect;

  const PixelRect top_rect = GetTopWidgetRect(widget_rect, top_widget);
  if (HaveTopWidget()) {
    top_widget->Move(top_rect);
    visible_rect = GetMapRectBelow(visible_rect, top_rect);
  ...
  if (HaveBottomWidget()) {
    ...
    visible_rect = GetMapRectAbove(visible_rect, bottom_rect);
  ...
  /* keep the map overlays (compass, map scale, final glide bar, ...)
     inside the visible part, even if the map extends beyond it */
  const PixelRect map_rc = map->GetPosition();
  const PixelRect safe_rc = ClipToSafeArea(map_rc, visible_rect);
