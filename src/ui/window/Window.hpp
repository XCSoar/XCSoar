...
public:
   ...
   */
  void TabStop() noexcept { tab_stop = true; }
   ...
   */
  void ControlParent() noexcept { control_parent = true; }

  /** The window has a thin-line border. */
  void Border() noexcept { has_border = true; }

  /** The window has a sunken 3D border. */
  void SunkenEdge() noexcept { Border(); }
...
protected:
  ...
  bool HasBorder() const noexcept { return has_border; }

public:
  bool IsDefined() const noexcept { return size.width > 0; }
  ...
  void MoveToCenter() noexcept {
    ...
    int dialog_x = parent_rect.left +
                   (int(parent_size.width) - int(window_size.width)) / 2;
    int dialog_y = parent_rect.top +
                   (int(parent_size.height) - int(window_size.height)) / 2;
...
public:
  ...
  bool IsInside(PixelPoint p) const noexcept {
    ...
    return unsigned(p.x) < size.width && unsigned(p.y) < size.height;
  }
