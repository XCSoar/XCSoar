...
RichTextWindow::LoadImage(const std::string &url) const noexcept
{
  ...
  if (StringStartsWith(url.c_str(), "resource:")) {
    ...
    /* Prefer the _RGBA variant (PNG with alpha channel) over the
       base resource (BMP with white background) so that images
       composite correctly on non-white backgrounds. */
