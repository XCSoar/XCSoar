                            ...
                            [[maybe_unused]] const PixelRect &rc) noexcept
{
  ...
  /* Always request elevation from devices when opening dialog to get latest value */
  if (backend_components->devices) {
    MessageOperationEnvironment env;
    backend_components->devices->RequestElevation(env);

    /* Wait up to 1000ms for the device to respond, reading directly from device blackboard */
    for (unsigned i = 0; i < 10; ++i) {
      Sleep(100);
      
      /* Read directly from device blackboard to get latest data */
  ...
  if (elevation_available) {
    elevation_control = AddInteger(_("Elevation"), _("Ground elevation for device calibration (meters)."),
                                    _T("%d m"), _T("%d"), -500, 9000, 1, elevation, this);
  } else {
