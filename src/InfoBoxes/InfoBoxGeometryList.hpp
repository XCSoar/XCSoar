...
/**
 * InfoBox geometries offered in setup and by the InfoBoxGeometry
 * input event.  Keep this as the single list — both callers share it.
 */
inline constexpr StaticEnumChoice info_box_geometry_list[] = {
...
static constexpr unsigned INFO_BOX_GEOMETRY_COUNT =
  ARRAY_SIZE(info_box_geometry_list) - 1;

/**
 * Index of @p geometry in #info_box_geometry_list, or 0 if the value
 * is obsolete / unknown (profile migration).
 */
[[gnu::pure]]
constexpr unsigned
FindInfoBoxGeometryIndex(InfoBoxSettings::Geometry geometry) noexcept
{
  for (unsigned i = 0; i < INFO_BOX_GEOMETRY_COUNT; ++i)
