...
ActionInterface::SetBallast(double ballast_litres, bool to_devices) noexcept
{
  ...
  if (to_devices && backend_components->devices) {
    ...
    if (plane.empty_mass > 0) {
      ...
      LogFmt("XCSoar: Calculated ballast overload: {:.3f} (dry_mass={:.2f}kg + ballast={:.2f}L) / ref_mass={:.2f}kg",
             overload, dry_mass, ballast_litres, plane.polar_shape.reference_mass);
