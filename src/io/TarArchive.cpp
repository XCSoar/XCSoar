...
TarReader::ReadData(OutputStream &out,
                    const std::function<void(uint64_t)> &on_bytes)
{
  const uint64_t size = remaining;
  CopyBytes(input, out, size, on_bytes);
  SkipBytes(input, PadToBlockSize(size));
  remaining = 0;
}

void
TarReader::Skip(const std::function<void(uint64_t)> &on_bytes)
{
  const uint64_t size = remaining;
  SkipBytes(input, size, on_bytes);
  SkipBytes(input, PadToBlockSize(size));
  remaining = 0;
}
