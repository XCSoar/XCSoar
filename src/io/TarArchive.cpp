...
bool
ReadHeaderBlock(FileReader &in, TarHeader &header) noexcept
{
...
void
SkipBytes(FileReader &in, uint64_t size)
{
  if (size > 0)
    in.Skip(static_cast<off_t>(size));
}

void
CopyBytes(FileReader &in, FileOutputStream &out, uint64_t size)
{
...
void
WriteHeaderBlock(FileOutputStream &out, std::string_view name, uint64_t size)
{
  TarHeader header{};
...
void
WritePaddingBytes(FileOutputStream &out, uint64_t size)
{
...
void
WriteEndMarker(FileOutputStream &out)
{
  TarBlock zero{};
...
void
TarReader::ReadData(FileOutputStream &out)
{
...
void
TarWriter::Add(std::string_view name, FileReader &in, uint64_t size)
{
