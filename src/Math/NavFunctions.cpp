...
void
xLL_to_XY(double Lat_TP, double Long_TP, double Lat_Pnt, double Long_Pnt,
    double *X, double *Y)
{
  double Delta_Long;
  double sin_TP_Lat;
  ...
  Denom = 1.0 + sin_TP_Lat * sin_Lat_Pnt
      + cos_TP_Lat * cos_Lat_Pnt * cos_Delta_Long;

  // xrTP_Position(&X_Own, &Y_Own);
  *X = (double)(((EARTH_DIAMETER * cos_Lat_Pnt * sin_Delta_Long) / Denom));

  *Y = (double)((((EARTH_DIAMETER * (cos_TP_Lat * sin_Lat_Pnt - sin_TP_Lat
      * cos_Lat_Pnt * cos_Delta_Long)) / Denom)));
}

void
xXY_Brg_Rng(double X_1, double Y_1, double X_2, double Y_2, double *Bearing,
    double *Range)
{
  double Rad_Bearing;
  double Rad_360 = 2 * M_PI;
...
void
xBrg_Rng_XY(double X_RefPos, double Y_RefPos, double Bearing, double Range,
    double *X, double *Y)
{
  (void)X_RefPos;
  (void)Y_RefPos;
...
void
xCrs_Spd_to_VxVy(double Crs, double Spd, double *Vx, double *Vy)
{
  double Crs_rad;
...
void
xVxVy_to_Crs_Spd(double Vx, double Vy, double *Crs, double *Spd)
{
  double Tmp_Spd;
...
void
LL_to_BearRange(double Lat_TP, double Long_TP, double Lat_Pnt, double Long_Pnt,
    double *Bearing, double *Range)
{
  double pos_X, pos_Y;

  xLL_to_XY(Lat_TP, Long_TP, Lat_Pnt, Long_Pnt, &pos_X, &pos_Y);
