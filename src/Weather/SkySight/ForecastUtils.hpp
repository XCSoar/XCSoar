...
/**
 * The deployed API selects a forecast run from this boundary.  Starting in
 * the past can select a historical run which contains no future steps.
 */
[[nodiscard]] inline time_t
GetForecastCatalogStart(time_t now) noexcept
{
  return now;
}

[[nodiscard]] inline bool
HasForecastCatalogLinks(const Layer &layer) noexcept
{
  return std::any_of(layer.forecast_datafiles.begin(),
