...
NavigatorRenderer::DrawText(
  ...
  // PREPARE TEXTS
  const RoughTimeDelta utc_offset1{CommonInterface::GetComputerSettings().utc_offset};
  ...
  if (tp == TaskType::ORDERED && has_started && basic.time_available) {
    ...
    const BasicStringBuffer<TCHAR, 8> time_elapsed_s_tmp = FormatLocalTimeHHMM(time_elapsed, utc_no_offset);
  ...
  if (tp == TaskType::ORDERED && has_started && basic.time_available)
    time_start_s.Format(_T("%s"), FormatLocalTimeHHMM(time_start, utc_offset1).c_str());
  else
  ...
  if (basic.time_available) {
    const BasicStringBuffer<TCHAR, 8> time = FormatLocalTimeHHMM(basic.time, utc_offset1);
  ...
  if (tp == TaskType::ORDERED && has_started && basic.time_available) {
    time_planned = TimeStamp{FloatDuration{calculated.ordered_task_stats.total.time_planned}};
    time_planned_s.Format(_T("%s"), 
                          FormatLocalTimeHHMM(time_planned, utc_no_offset).c_str());
  } else if (tp != TaskType::ORDERED && basic.time_available) {
    time_planned = TimeStamp{FloatDuration{calculated.task_stats.total.time_planned}};
    time_planned_s.Format(_T("%s"), 
                          FormatLocalTimeHHMM(time_planned, utc_no_offset).c_str());
  } else
  ...
  if ((tp == TaskType::ORDERED && has_started) && basic.time_available) {
    arrival_planned = TimeStamp{FloatDuration{time_start.ToDuration() + time_planned.ToDuration()}};
    arrival_planned_s.Format(_T("%s"), 
                             FormatLocalTimeHHMM(arrival_planned, utc_offset1).c_str());
  } else if (tp != TaskType::ORDERED && basic.time_available) {
    arrival_planned = TimeStamp{FloatDuration{basic.time.ToDuration() + time_planned.ToDuration()}};
    arrival_planned_s.Format(_T("%s"), 
                             FormatLocalTimeHHMM(arrival_planned, utc_offset1).c_str());
  } else
  ...
  if (tp == TaskType::ORDERED) {
    waypoint_distance = calculated.ordered_task_stats.current_leg.vector_remaining.distance;
  } else {
  ...
  if (tp == TaskType::ORDERED) {
    ...
    waypoint_altitude_diff = calculated.task_stats
                               .current_leg.solution_remaining.GetRequiredAltitude();
  }
  FormatAltitude(waypoint_altitude_diff_s.data(), 
  ...
  if (canvas.GetWidth() > canvas.GetHeight() * 6.1)
    infos_next_waypoint_s.Format(_T("%s   %s  %s  %s"), 
    ...
    infos_next_waypoint_s.Format(_T("%s   %s  %s"), 
                                 waypoint_distance_s.c_str(),
                                 waypoint_altitude_diff_s.c_str(), 
                                 waypoint_GR_s.c_str());
  else
    infos_next_waypoint_s.Format(_T("%s   %s"), 
                                 waypoint_distance_s.c_str(), 
                                 waypoint_altitude_diff_s.c_str());
  ...
  infos_next_waypoint_unit_GR_s.Format(_T("%s   %s  %s"), waypoint_distance_s.c_str(),
                                       waypoint_altitude_diff_s.c_str(), 
                                       waypoint_GR_s.c_str());
  ...
  StaticString<5> sz_tmp_current_speed_s{"0"};
  ...
  psSize = {static_cast<int>(rc_width), static_cast<int>(rc_height)};
  prRect = {ppOrigin, psSize};
  canvas.DrawClippedText(pxpt_pos_average_speed,
                         prRect,
