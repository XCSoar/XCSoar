    ...
    [[maybe_unused]] const InfoBoxLook &look_infobox) noexcept
{
  ...
  } else time_elapsed_s.Format(_T("%s"), _T("--:--"));
  ...
  else time_start_s.Format(_T("%s"), _T("--:--"));
  ...
  if (tp == TaskType::ORDERED && has_started && basic.time_available) {
    ...
    time_planned_s.Format(
        _T("%s"), FormatLocalTimeHHMM(time_planned, utc_no_offset).c_str());
  } else time_planned_s.Format(_T("%s"), _T("--:--"));
  ...
  if ((tp == TaskType::ORDERED && has_started) && basic.time_available) {
    ...
    arrival_planned_s.Format(
        _T("%s"), FormatLocalTimeHHMM(arrival_planned, utc_offset1).c_str());
  } else arrival_planned_s.Format(_T("%s"), _T("--:--"));
  ...
  // snprintf(waypoint_name_s, ARRAY_SIZE(waypoint_name_s), _T("%s"),
  // wp_current.name.c_str());
  ...
  // FormatUserDistance(
  //   waypoint_distance, waypoint_distance_s, true,
  //   precision_waypoint_distance);
  ...
  if (!basic.track_available) bearing_diff.Zero();
  ...
  else times_arrival_planned_s.Format(_T("%s"), arrival_planned_s.c_str());
  ...
  Font font;
  // TODO: replace this ugly ratio_dpi by a better implementation. through
  // NavigatorLook? see DialogLook.cpp / InfoBoxLook.cpp this magic number
  // allows to scale correctly all texts even if with custom dpi
  ...
  if (!look_nav.inverse) canvas.SetTextColor(COLOR_BLACK);
  else canvas.SetTextColor(COLOR_WHITE);
  ...
  else font_height = rc_height * 26 / 200;
  ...
  if (canvas.GetWidth() > canvas.GetHeight() * 2.3) {
    ...
    const PixelPoint pxpt_pos_current_speed{pos_x_speed_altitude,
                                            pos_y_current_speed_s};
    canvas.Select(font);

    ppOrigin = {0, 0};
    psSize = {static_cast<int>(rc_width), static_cast<int>(rc_height)};
    prRect = {ppOrigin, psSize};
    canvas.DrawClippedText(pxpt_pos_current_speed, prRect, current_speed_s);
    ...
    const PixelPoint pxpt_pos_altitude{pos_x_speed_altitude,
                                       pos_y_current_altitude_s};
    canvas.Select(font);

    psSize = {static_cast<int>(rc_width), static_cast<int>(rc_height)};
    prRect = {ppOrigin, psSize};
    canvas.DrawClippedText(pxpt_pos_altitude, prRect, current_altitude_s);
  ...
  else font_height_waypoint = rc_height * 30 / 200;
  ...
  if (canvas.GetWidth() > canvas.GetHeight() * 3.4)
    font_height = rc_height * 35 / 200;
  else font_height = rc_height * 24 / 200;
  ...
  else size_text_tmp = size_text.width / 2;
    ...
    unsigned task_size, [[maybe_unused]] const NavigatorLook &look) noexcept
{
  ...
  //     const GlideResult result_wp_before =
  //     mac_cready.SolveStraight(state_wp_before);
  ...
  //   "
  //   // rtrtr :" << waypoint_current->elevation << std::endl; std::cout <<
  //   "cqscn
  //   // "
  //   // << waypoint_before->name << "  rtrtr :" << waypoint_before->elevation
  //   <<
  //   // std::endl; WaypointReachability wr =
