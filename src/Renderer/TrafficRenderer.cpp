...
void
TrafficRenderer::DrawAircraftTypeIcon(Canvas &canvas,
                                      ...
                                      const Color color) noexcept
{
  const MaskedIcon &icon =
    traffic_look.GetAircraftTypeIcon(traffic.type);

  icon.Draw(canvas, pt, icon_height, angle, color);
}
...
{
  if (!fading && TrafficLook::aircraft_type_icons) {
    DrawAircraftTypeIcon(canvas, traffic_look, traffic, angle, pt,
                         Layout::Scale(40),
                         traffic_look.GetTrafficDisplayColor(traffic));
