...
std::vector<std::string> GetUserRepositoryURIs()
{
  ...
  for (auto i : TIterableSplitString(src, separator[0] )) {
    if (i.empty()) continue;

#ifdef _UNICODE
    std::wstring temp(i.data(), i.size());
    const WideToUTF8Converter converted(temp.c_str());
    if (converted.IsValid())
      uris.emplace_back(converted);
#else
...
EnqueueRepositoryDownload(bool force)
{
  ...
  for (const auto &uri : uris) {
    if(uri.empty())
      continue;
    StringFormat(filename, 32, "user_repository_%d", user_repository_index++);
