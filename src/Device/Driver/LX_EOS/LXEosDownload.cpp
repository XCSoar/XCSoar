                            ...
                            OperationEnvironment& env)
{
  ...
  try {
    ...
    // Maximum number of flights to be obtained is limited by length of the
    // list
...
LXEosDevice::GetNumberOfFlights(OperationEnvironment& env)
{
  ...
  WriteAndWaitForACK(std::as_bytes(std::span{&data, sizeof(data)}), env);
  ...
  // Check CRC
  if (UpdateCRC8(std::as_bytes(std::span{ &response, sizeof(response) }),
...
LXEosDevice::GetFlightInfo(const uint8_t index, OperationEnvironment& env)
{
  ...
  data.crc = UpdateCRC8(std::as_bytes(std::span{ &data, sizeof(data) - 1 }),
                        std::byte{ 0xFF });
  WriteAndWaitForACK(std::as_bytes(std::span{ &data, sizeof(data) }), env);
  ...
  if (UpdateCRC8(std::as_bytes(std::span{ &info, sizeof(info) }),
                               ...
                               OperationEnvironment& env)
{
  ...
  crc = UpdateCRC8(std::as_bytes(std::span{ &header, sizeof(header) }), crc);
  crc =
