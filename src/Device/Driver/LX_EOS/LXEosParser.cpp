...
LXEosDevice::LXWP2(NMEAInputLine& line, NMEAInfo& info)
{
  ...
  if (vario_settings.dry_mass_to_ref_mass_uptodate) {
    float ballast_overload = bal / vario_settings.dry_mass_to_ref_mass;

    // Negative result can happen when the empty weight and/or crew weight
    // setting in XCSoar is higher than in the vario
    if (ballast_overload < 1)
      ballast_overload = 1;
