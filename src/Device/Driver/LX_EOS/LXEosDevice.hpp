...
{
  const uint8_t ack = 0x06;
  uint16_t uiFlightID; // Flight id (not used, index is used for downloading!)
  ...
  uint16_t uiMaxALT; // Maximum altitude (not used)
  uint16_t uiMaxIAS; // Maximum indicated air speed (not used)
...
struct VarioSettings
{
  ...
  float dry_mass_to_ref_mass =
    1; // Ratio of glider's dry mass to reference mass of device's polar
  GlidePolar xcsoar_polar; // Polar coefficients used by XCSoar
  bool dry_mass_to_ref_mass_uptodate = false;
};
...
private:
   ...
   * @param settings
   */
  static void CalculateDryMassToReferenceMassRatio(VarioSettings& settings);
  ...
  const std::chrono::_V2::steady_clock::duration communication_timeout =
