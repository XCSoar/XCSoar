                         ...
                         OperationEnvironment& env)
{
  ...
  data.uiTpNr = tp_nr;
  data.uiR1 = ToLE32(turnpoint.radius);
  ...
  data.crc = UpdateCRC8(
   std::as_bytes(std::span{ &data, sizeof(data) - 1}), std::byte{0x44});
  return WriteAndWaitForACK(std::as_bytes(std::span{ &data, sizeof(data)}), env);
}
