                         ...
                         OperationEnvironment& env)
{
  ...
  data.crc = UpdateCRC8(std::span{reinterpret_cast<std::byte*>(&data),sizeof(data)-1}, std::byte{0x44});
  return WriteAndWaitForACK(std::span{reinterpret_cast<std::byte*>(&data),sizeof(data)}, env);
}
  ...
  OperationEnvironment& env)
{
  ...
  return WriteAndWaitForACK(std::span{ data_bytes_pointer, sizeof(data) },
                            env);
}
