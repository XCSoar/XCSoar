...
LXEosDevice::LinkTimeout()
{
  ...
  vario_settings.dry_mass_to_ref_mass_uptodate = false;
}
...
{
  const std::lock_guard lock{ settings_mutex };
  if (vario_settings.dry_mass_to_ref_mass_uptodate) {
    vario_settings.bal = overload * vario_settings.dry_mass_to_ref_mass;
    return SendNewSettings(env);
  }
  return false;
}
                                ...
                                const DerivedInfo& calculated)
{
  ...
  if (!ComparePolarCoefficients(old_polar.GetCoefficients(),
    ...
    CalculateDryMassToReferenceMassRatio(vario_settings);
  }
}
...
LXEosDevice::HasReliableAltOffset(DeviceInfo device)
{
  if (device.product == "LX Eos"sv) {
    if (device.software_version == "1.7"sv)
      return true;
  }
...
LXEosDevice::CalculateDryMassToReferenceMassRatio(VarioSettings& settings)
{
  ...
  float device_reference_mass =
    settings.xcsoar_polar.GetReferenceMass() * v_ratio * v_ratio;

  settings.dry_mass_to_ref_mass =
    settings.xcsoar_polar.GetDryMass() / device_reference_mass;
  settings.dry_mass_to_ref_mass_uptodate = true;
}
