 ...
 * Konvertierroutinen
 *
 * vom bin�ren Volkslogger-Format  GCS ins IGC-Format
...
public:
  ...
  print(int version, FILE *aus)
  {
    ...
    if (hasdeclaration) {
      ...
      if (TID > 9999)
        TID = 9999; // Gr��enbegrenzungen wg. Ausdruck
...
struct IGCHEADER
{
  ...
  output(int version, int oo_fillin)
  {
    ...
    igc_filter(A);
    if ((version < 413) || (version >= 416)) { // aus Kompatibilit�t zu alten Versionen
...
/*
Bin�ren Datenblock *bin_puffer in das IGC-Format umwandeln und in der
Datei *Ausgabedatei speichern


Konvertierung erfolgt in 2 Phasen:
1) Auff�llen der HFxxx-Records mit den Daten aus GCS-Datei
   Auff�llen der C-Records mit den Daten aus der GCS-Datei
2) Ausdrucken der Hxxxx-Records in der vorgeschriebenen Reihenfolge,
   ...
   Konvertierung und Ausdrucken der B- und E-Datens�tze

Bin�rdatensatztypen (Achtung: aktuelles Datenformat siehe Firmwarelisting):
  T mit rel. Zeit  / ohne rel. Zeit
  V variable L�nge / feste L�nge
     TV  L
...
Parameter
  igcfile_version:
    ...
    dies ist aus Kompatibilit�tsgr�nden erforderlich
  Ausgabedatei:
    Datei, in die das Ergebnis der Konvertierung (IGC-Datei) geschrieben
    wird
  bin_puffer:
    Zeiger auf den Speicherbereich, in dem die bin�re formatierte IGC-
  ...
  *sp:
    Position, an der die Signatur in der Bin�rdatei liegt
*/
