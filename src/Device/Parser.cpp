...
NMEAParser::RMC(NMEAInputLine &line, NMEAInfo &info)
{
 ...
 //  if (track_available) {
  ...
  if (variation_available) {
    // OK to show heading on the ground
...
/**
 * Parses magnetic heading non-negative floating-point angle value
 * in degrees.
 */
static bool
ReadMagHeading(NMEAInputLine &line, Angle &value_r)
{
  fixed value;
  if (!line.ReadChecked(value))
    return false;
...
NMEAParser::HDM(NMEAInputLine &line, NMEAInfo &info)
{
  ...
  Angle heading;
  bool heading_available = ReadMagHeading(line, heading);
