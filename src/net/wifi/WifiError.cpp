...
FormatWifiErrorForUser(const char *what)
{
  ...
  if (w == std::string_view{"No D-Bus connection"})
    return {_("No D-Bus connection")};

  if (w == std::string_view{"ConnMan is not available"})
    return {_("ConnMan is not available")};

  if (w == std::string_view{"No Wi-Fi backend available"})
    return {_("No Wi-Fi backend available")};

  if (w == std::string_view{"NetworkManager is not available"})
