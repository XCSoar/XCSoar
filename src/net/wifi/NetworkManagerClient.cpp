...
static std::string
FindSavedConnectionPath(ODBus::Connection &c, const std::string &ssid_wanted)
{
  for (const auto &saved : NmClient::ListSavedConnections(c))
    if (saved.ssid_text == ssid_wanted)
      return saved.path;
                                    ...
                                    const std::string &ssid) noexcept
{
  try {
    ...
    return !FindSavedConnectionPath(c, ssid).empty();
  } catch (...) {
    return false;
  }
}
...
NmClient::Remove(ODBus::Connection &c, const char *connection_path)
{
  if (connection_path == nullptr || *connection_path == 0)
    throw std::runtime_error{"Missing NetworkManager connection path"};
                  ...
                  const char *wpa2_psk_or_null)
{
  ...
  if (pw != nullptr) {
    ...
    const std::string saved = !ap.saved_path.empty()
      ? ap.saved_path
      : FindSavedConnectionPath(c, ap.ssid_text);
    if (!saved.empty()) {
