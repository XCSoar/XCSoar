...
FindSavedConnection(const std::vector<NmClient::SavedConnection> &saved_connections,
                    const NmClient::AccessPoint &ap) noexcept
{
  if (ap.ssid_text.empty())
    return nullptr;

  const auto found = std::find_if(saved_connections.begin(),
                                  saved_connections.end(),
                                  [&ap](const auto &saved) {
                                    return !saved.ssid_text.empty() &&
                                      saved.ssid_text == ap.ssid_text;
                                  });
  return found != saved_connections.end()
    ? &*found
    : nullptr;
}
                          ...
                          const std::vector<NmClient::SavedConnection> &saved_connections) noexcept
{
  for (auto &ap : access_points)
    if (const auto *saved = FindSavedConnection(saved_connections, ap); saved != nullptr)
                      ...
                      const NmClient::SavedConnection &saved) noexcept
{
  for (std::size_t i = 0; i < count; ++i)
    if (entries[i].is_visible && !saved.ssid_text.empty() &&
        entries[i].ssid == saved.ssid_text.c_str())
      return true;
  ...
  try {
    ...
    for (const auto &ap : access_points) {
      ...
      AssignNetworkEntryFromAccessPoint(candidate, ap,
                    FindSavedConnection(saved_connections, ap),
                    active);
      if (active)
