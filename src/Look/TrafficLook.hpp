...
struct TrafficLook
{
  ...
  const Brush& GetBasicTrafficBrush(const TrafficClimbAltIndicators &indicators) const;
  const Brush& GetColourfulTrafficBrush(const TrafficClimbAltIndicators &indicators) const;
};
