...
void
Server::OnPing(const boost::asio::ip::udp::endpoint &sender,
               uint64_t key,
               unsigned id)
{
  SendPacket(sender, MakeAck(key, id, 0));
}

inline void
Server::OnDatagramReceived(const boost::asio::ip::udp::endpoint &sender,
                           void *data, size_t length)
{
  ...
  case PING:
    OnPing(sender, FromBE64(header.key), FromBE16(ping.id));
    break;

  case FIX:
    OnFix(sender, FromBE64(header.key),
  ...
  case TRAFFIC_REQUEST:
    OnTrafficRequest(sender, FromBE64(header.key),
                     traffic.flags & ToBE32(TrafficRequestPacket::FLAG_NEAR));
    break;

  case USER_NAME_REQUEST:
    OnUserNameRequest(sender, FromBE64(header.key),
                      FromBE32(user_name.user_id));
    break;

  case WAVE_SUBMIT:
    OnWaveSubmit(sender, FromBE64(header.key),
  ...
  case WAVE_REQUEST:
    OnWaveRequest(sender, FromBE64(header.key));
    break;

  case THERMAL_SUBMIT:
    OnThermalSubmit(sender, FromBE64(header.key),
  ...
  case THERMAL_REQUEST:
    OnThermalRequest(sender, FromBE64(header.key));
    break;
...
Server::OnReceive(const boost::system::error_code &ec, size_t size)
{
  ...
  OnDatagramReceived(sender_endpoint, buffer, size);
