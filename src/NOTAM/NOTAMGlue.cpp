                ...
                const std::vector<std::string> &removed_ids)
{
  ...
  if (!removed.empty() || !updated.empty()) {
    api_items.erase(std::remove_if(api_items.begin(), api_items.end(),
                                   [&removed, &updated](const boost::json::value &item) {
                                     std::string id;
...
NOTAMGlue::LoadNOTAMsInternal(GeoPoint location, bool force_network)
{
  ...
  const auto clear_known_if_current = [this, request_generation]() {
    const std::lock_guard<Mutex> lock(mutex);
...
NOTAMGlue::HandleLoadComplete(std::exception_ptr error) noexcept
{
  ...
  const auto notify_load_complete = [this](bool success) {
    {
      const std::lock_guard<Mutex> lock(mutex);
      listener_load_complete_pending = success;
      if (!success)
        manual_refresh_requested = false;
    }
  ...
  const auto apply_committed_load = [this, notify_load_complete]() {
    ...
    bool show_loaded_message = false;
    {
      ...
      show_loaded_message = manual_refresh_requested;
      manual_refresh_requested = false;
    }
    listener_notify.SendNotification();

    if (show_loaded_message)
      AddLoadedNOTAMsMessage(loaded_count);
  };
...
NOTAMGlue::OnListenerNotification() noexcept
{
  ...
  if (notify_load_complete.has_value())
    listeners.ForEach([success = *notify_load_complete](auto *listener) {
      listener->OnNOTAMsLoadComplete(success);
    });
}
...
NOTAMGlue::UpdateAirspaces(Airspaces &airspaces)
{
  ...
  // Save all airspaces that were not injected by NOTAMGlue
  std::vector<AirspacePtr> saved_airspaces;
  for (const auto &airspace : airspaces.QueryAll()) {
    const auto &current = airspace.GetAirspace();
    if (impl->injected_airspaces.find(&current) ==
        impl->injected_airspaces.end()) {
  ...
  std::unordered_set<const AbstractAirspace *> temp_injected_airspaces;
  unsigned added_count = 0;
  unsigned filtered_count = 0;

  try {
    // Restore non-injected airspaces into a temporary tree first.
    ...
    for (const auto &notam : impl->current_notams) {
      try {
        ...
        temp_injected_airspaces.insert(ptr);
        added_count++;
      } catch (const std::exception &e) {
    ...
    // Optimize the airspace tree before replacing the live one.
  ...
  airspaces.Clear();
  for (const auto &airspace : temp_airspaces.QueryAll())
    airspaces.Add(airspace.GetAirspacePtr());
  airspaces.Optimise();
  impl->injected_airspaces = std::move(temp_injected_airspaces);
