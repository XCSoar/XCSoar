...
NOTAMPointToGeoPoint(const NOTAMPoint &point) noexcept
{
  if (!std::isfinite(point.latitude) || point.latitude < -90 ||
      point.latitude > 90 ||
      !std::isfinite(point.longitude) || point.longitude < -180 ||
      point.longitude > 180)
    return std::nullopt;
  ...
  try {
    ...
    switch (notam.geometry.type) {
      case NOTAM::NOTAMGeometry::Type::POINT:
        ...
        if (const auto center = NOTAMPointToGeoPoint(notam.geometry.center);
            center) {
      ...
      case NOTAM::NOTAMGeometry::Type::POLYGON:
        if (notam.geometry.polygon_points.size() >= 3) {
          ...
          for (const auto &notam_point : notam.geometry.polygon_points) {
            if (const auto point = NOTAMPointToGeoPoint(notam_point);
                point) {
