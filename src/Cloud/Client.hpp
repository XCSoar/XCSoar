    ...
    boost::intrusive::unordered_set_base_hook<boost::intrusive::link_mode<boost::intrusive::normal_link>>
{
  ...
  struct KeyEqual {
    ...
    gcc_pure
    bool operator()(uint64_t a, const Client &b) const {
      return a == b.key;
    }
  };
  ...
  struct IdCompare {
    ...
    gcc_pure
    bool operator()(unsigned a, const Client &b) const {
      return a < b.id;
    }
  };

  Client(const boost::asio::ip::udp::endpoint &_endpoint, uint64_t _key,
         unsigned _id,
 ...
 */
struct ClientIndexable {
  typedef GeoPoint result_type;

  gcc_pure
  result_type operator()(const ClientPtr &client) const {
    return client->location;
  }
};

class ClientContainer {
  typedef boost::geometry::index::rtree<ClientPtr, boost::geometry::index::rstar<16>,
                                        ClientIndexable> Tree;

  typedef boost::intrusive::list<Client,
                                 boost::intrusive::constant_time_size<false>> List;

  typedef boost::intrusive::unordered_set<Client,
                                          boost::intrusive::hash<Client::KeyHash>,
                                          boost::intrusive::equal<Client::KeyEqual>,
                                          boost::intrusive::constant_time_size<false>> KeySet;

  typedef boost::intrusive::set<Client,
                                boost::intrusive::compare<Client::IdCompare>,
  ...
  /**
   * Map (secret) key to #Client.
   */
  KeySet key_set;

  /**
   * Map public id to #Client.
   */
  IdSet id_set;

  /**
   * The public id assigned to the next new #Client.
   */
  unsigned next_id = 1;
...
public:
   ...
   */
  gcc_pure
  Client *Find(uint64_t key);

  /**
   * Create a new #Client, or refresh the existing one.
   */
  Client &Make(const boost::asio::ip::udp::endpoint &endpoint,
  ...
  /**
   * Remove a #Client and its data.  Be careful - the given reference
   * is invalidated, unless the caller holds another #ClientPtr.
   */
