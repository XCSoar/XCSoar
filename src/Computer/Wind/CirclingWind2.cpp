...
CirclingWind2::CalcWind(double quality_metric, const size_t n_samples, [[ maybe_unused ]] const Angle circle)
{
  ...
  if (min_fit_metric > (INITIAL_min_fit_metric - 1)) {
    LogString("CirclingWind doesn't converge");
  ...
  if (min_fit_metric > 100) quality -= 1; // penalty for bad fit
  if (min_fit_metric < 15)  quality += 1; // bonus for good fit
  if (min_fit_metric < 5)   quality += 1; // extra bonus for very good fit
