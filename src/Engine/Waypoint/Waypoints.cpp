...
Waypoints::GenerateTempPoint(const GeoPoint& location, const double terrain_alt,
                             const TCHAR *name) const noexcept
{
  assert(name != nullptr);
  ...
  to_point.name = name;
  to_point.shortname = name;  // Use same value for both name and shortname

  const bool is_takeoff = StringIsEqual(name, _T("(takeoff)"));
  to_point.type = is_takeoff ? Waypoint::Type::OUTLANDING : Waypoint::Type::NORMAL;
