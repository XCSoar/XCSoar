  ...
  if constexpr (IsAndroid()) {
    ...
    for (auto &path : context->GetExternalFilesDirs(env)) {
      __android_log_print(ANDROID_LOG_DEBUG, "XCSoar",
                          "Context.getExternalFilesDirs()='%s'", path.c_str());
      auto xcsoarlog_path =
          AllocatedPath::Build(Path(path), Path("xcsoar.log"));
      if (File::Exists(xcsoarlog_path)) {
        /*
         * Old Android user will keep using getExternalFilesDirs() if they
         * already have data in it Otherwise we should default them to the new
