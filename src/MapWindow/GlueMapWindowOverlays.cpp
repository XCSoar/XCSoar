...
GlueMapWindow::DrawSystemStatus(Canvas &canvas, const PixelRect &rc) const noexcept
{
  ...
  if (gps_icon != nullptr) {
    ...
    if (gps_txt != nullptr) {
      ...
      text_p.y += std::max(0, static_cast<int>(gps_icon->GetSize().height - font.GetHeight()) / 2);
                               ...
                               const PixelRect &rc) const noexcept
{
  ...
  tb_rect.top = rc.top + Layout::Scale(30);
  tb_rect.bottom = rc.top + rc.GetHeight() / 5 + Layout::Scale(26);
