...
separate output directory (`output/OPT/bin/`). Windows OpenGL builds
(`WIN64OPENGL`, `WIN32OPENGL`) keep their own output directory
...
Before a pull request, build them with `everything` so they compile.
