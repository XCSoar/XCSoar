...
focusing on common workflows and useful commands. For detailed Git
documentation, see the official Git manual or `git help <command>`.
...
- **Always rebase on upstream/master** before submitting PRs to keep history
  clean
...
- **Write good commit messages** - follow the project’s format (see
  [Policy](/dev/policy))
- **Use visual tools** (tig, lazygit) for complex operations - easier than
  command line
