...
tools), see [Debugging Tips](/dev/debugging).
...
[Development workflow](/dev/build#development-workflow) in [Compiling
XCSoar](/dev/build)). They are defined in `build/test.mk` and compiled as the
`debug` make target.
