---
title: Checklists
description: Custom checklists with Markdown formatting, clickable links, interactive checkboxes, the supported URI schemes and an example file.
---

XCSoar supports custom checklists with Markdown formatting, clickable links, and
...
1. The file specified in the `Site Files` → `Checklist` setting
2. `xcsoar-checklist.txt` in the XCSoar data directory (typically
   `checklists/xcsoar-checklist.txt`; see [Data directory](/dev/data-directory))
...
Each checklist can have multiple pages. Pages are defined by section headers in
square brackets:
...
Ticks are remembered while XCSoar is running. Closing and reopening the dialog
keeps them. They reset when you select a different checklist file in Site Files,
or when you restart XCSoar. Editing the page text in the file also drops that
page’s saved ticks.
...
Set the COM standby or active frequency in XCSoar (and forward it to compatible
devices when configured). The link must end with `#standby` or `#active` after
the MHz value. Use a markdown link `[name](vhf:…#…)` so the station or service
name is stored for the radio; the on-screen label combines name and frequency. A
short confirmation appears after a successful tap; invalid links show a warning.
