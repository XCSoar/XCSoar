...
2. `xcsoar-checklist.txt` in the XCSoar data directory (typically
   `checklists/xcsoar-checklist.txt`; see
   [XCSoar data directory](/dev/data-directory))
...
Interactive checkboxes that can be toggled by clicking or pressing
Enter:
...
XCSoar supports various link types that can be clicked to perform
actions.
...
Set the COM **standby** or **active** frequency in XCSoar (and forward it
to compatible devices when configured). The link must end with
`#standby` or `#active` after the MHz value. Use a markdown link
`[name](vhf:…#…)` so the station or service name is stored for the
radio; the on-screen label combines name and frequency. A short
confirmation appears after a successful tap; invalid links show a
warning.
