...
This guide documents how to rebrand XCSoar to another name, making it easier to
create and maintain forks.
...
::field-group{.name-code}
  ...
  The main product name (e.g., `"XCSoar"`). Can be overridden at build time.
  :::
...
::field-group{.name-code}
  ...
  User home directory on Linux/Unix. Derived from: `"." PRODUCT_NAME_LC` →
  `".xcsoar"`
  :::
::
...
The product name macros automatically generate appropriate paths for each
platform:
...
- `Data/XCSoar.manifest.in` - application manifest (identity name and version
  come from `PRODUCT_NAME` / `VERSION.txt`)
...
These can be kept as-is (they’re internal symbols) or changed manually if
desired.
...
- Data directory: `%LOCALAPPDATA%\MyGliderData` (Windows), `~/MyGliderData`
  (macOS), or `~/.myglider` (Linux)
...
The [OpenSoar](https://github.com/OpenSoaring/OpenSoar) fork demonstrates a
complete rebranding. Here’s what they changed:
...
**No.** These are internal C++ symbols and don’t affect the user-visible product
name. Most forks keep these as-is.
...
**No.** By keeping your changes in `src/ProductName.hpp`, you can merge upstream
XCSoar changes without conflicts. The header file is designed to be
fork-friendly.
...
Your fork will use `~/.xcsoar`, sharing the same config with XCSoar. This can be
useful for testing but may confuse users.
