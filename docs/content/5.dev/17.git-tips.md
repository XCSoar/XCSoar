...
description: Interactive rebase, fixup commits, useful one-liners, visual tools and common workflows for XCSoar development.
---

This document provides practical Git tips and tricks for XCSoar
development, focusing on common workflows and useful commands. For
detailed Git documentation, see the official Git manual or
`git help <command>`.

## Interactive Rebase

Interactive rebase (`git rebase -i`) is one of the most powerful tools
for cleaning up commit history before submitting a pull request.
...
Fixup commits are useful for making small corrections without creating
a new standalone commit.
...
- **Always rebase on upstream/master** before submitting PRs to keep
  history clean
- **Use fixup commits** for small corrections, then auto-squash before
  pushing
- **Test after rebase** - rebasing rewrites history and can introduce
  issues
- **Use `--force-with-lease`** instead of `--force` when pushing rebased
  branches
- **Keep commits focused** - one logical change per commit
- **Write good commit messages** - follow the project’s format (see
  [Policy](/dev/policy))
- **Use visual tools** (tig, lazygit) for complex operations - easier
  than command line
- **Don’t rebase shared branches** - only rebase your local feature
  branches

## Additional Resources

- [Policy](/dev/policy) - XCSoar development policy and Git workflow
