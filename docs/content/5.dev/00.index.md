...
Documentation for people **building or hacking on** XCSoar: compiling,
dev environment, architecture, policy, debugging utilities, Git tips, and
more.
...
- [Map data file formats](/dev/mapfile) — terrain, topography, airspace, and waypoint files XCSoar loads
- [Checklists](/dev/checklist) — checklist file format: paths, Markdown, checkboxes, and links (`https:`, `tel:`, `sms:`, `geo:`, `xcsoar://` dialogs, …)
- [Input events](/dev/input-events) — `.xci` mapping of keys, hardware buttons, and touch labels to actions; default F-key groups and glide-computer events
- [Lua scripting](/dev/lua) — Lua API inside XCSoar for automation (often used with input events)
