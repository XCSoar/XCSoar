...
def entry_block_start(lines: list[str], linenum: int) -> int:
  ...
  Walk upward through leading metadata: comments, msgctxt, and quoted
  continuation lines (wrapped msgctxt / msgid strings above linenum).
  """
  i = max(0, linenum - 1)
  while i > 0:
    prev = lines[i - 1]
...
def build_blocks(path: Path, po: polib.POFile) -> tuple[list[str], dict[POKey, EntryBlock]]:
  ...
  for i, entry in enumerate(entries):
    ...
    key = entry_key(entry)
    if key in blocks:
      raise ValueError(f"duplicate PO key at line {entry.linenum}: {key!r}")
    blocks[key] = EntryBlock(start=start, end=end, lines=lines[start:end])

  return lines, blocks


def write_normalized_fallback(original_path: Path, normalized_path: Path) -> None:
  """Replace original with normalized content, stripping `#:` locations."""
...
def main() -> int:
  ...
  try:
    ...
    # Duplicate keys: refuse silent overwrite; use normalized catalog.
    write_normalized_fallback(original_path, normalized_path)
    return 0
