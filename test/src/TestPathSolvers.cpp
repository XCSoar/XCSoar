...
static constexpr double DIST_EPS = 1e-6;
...
TestDijkstraMinPicksNearerFirstStage()
{
  ...
  ok1(equals(d.GetSolution(0).GetLocation().Distance(L),
             A.Distance(L)));
  ok1(std::fabs(d.GetSolution(1).GetLocation().Distance(C)) < DIST_EPS);
}
...
TestDijkstraMaxTwoStages()
{
  ...
  ok1(std::fabs(s0.Distance(A)) < DIST_EPS ||
      std::fabs(s0.Distance(B)) < DIST_EPS);
  ok1(std::fabs(d.GetSolution(1).GetLocation().Distance(C)) < DIST_EPS);
}
