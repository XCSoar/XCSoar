...
static MessagingRecord *
FindMessagingRecord(FlarmMessagingDatabase &db, const char *hex)
{
...
TestFlarmMessagingIO()
{
  ...
  MessagingRecord *mr = FindMessagingRecord(db2, "DDAED5");
  if (ok1(mr != nullptr)) {
...
TestFlarmMessagingFile()
{
  ...
  // Verify first record
  ok1(FindMessagingRecord(db, "DDAED5") != nullptr);

  // Verify second record with special characters
  MessagingRecord *mr = FindMessagingRecord(db, "DEADFF");
  ok1(mr != nullptr);
  if (mr != nullptr)
  ...
  ok1(FindMessagingRecord(db, "AABBCC") != nullptr);

  // Verify fourth record with whitespace
  ok1(FindMessagingRecord(db, "112233") != nullptr);
}
...
TestFlarmMessagingCycle()
{
  ...
  MessagingRecord *mr = db.FindRecordById(id);
  if (!ok1(mr != nullptr))
    return;
  ...
  // does not drop it.
  {
    ...
    MessagingRecord *mr2 = db.FindRecordById(id2);
    if (!ok1(mr2 != nullptr))
      return;
...
TestFlarmMessagingThreadSafety()
{
  ...
  // Scenario 2: Concurrent updates and file saves
  {
    ...
    MessagingRecord *mr = db2.FindRecordById(base.id);
    ok1(mr != nullptr);
    if (mr != nullptr) {
