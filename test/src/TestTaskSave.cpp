...
TestTaskSave()
{
  ...
  const auto *cyl0 = dynamic_cast<const CylinderZone *>(&z0);
  const auto *cyl1 = dynamic_cast<const CylinderZone *>(&z1);
  const auto *cyl2 = dynamic_cast<const CylinderZone *>(&z2);
  ok1(cyl0 != nullptr);
  ok1(cyl1 != nullptr);
  ok1(cyl2 != nullptr);
  ok1(equals(cyl0->GetRadius(), 500.));
  ok1(equals(cyl1->GetRadius(), 10000.));
  ok1(equals(cyl2->GetRadius(), 500.));
}
