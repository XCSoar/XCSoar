...
Contributor policy: `docs/content/en/5.dev/04.policy.md`. Developer docs:
`docs/content/en/5.dev/00.index.md`. Do not copy `.cursor/rules/` into this
file. Open the matching rule when the work matches the table below.

## Docs

Always read `docs/content/en/5.dev/06.architecture.md` before changing layers,
...
User-facing behaviour: update `docs/content/en/1.manual/` (not
`docs/content/en/5.dev/`) and `NEWS.txt`. See `.cursor/rules/user-manual.mdc`.
...
- User-visible strings: `_()`, `N_()`, `C_()`, `NC_()`. Reuse an existing
  ...
  `docs/content/en/5.dev/10.translations.md`. Log messages stay in English.
- User-visible behaviour: add a `NEWS.txt` bullet **and** update the English
  user manual in `docs/content/en/1.manual/`. Style:
  `.cursor/rules/news.txt.mdc` and `.cursor/rules/user-manual.mdc`.
...
- UI must work on OpenGL and memory canvas (Kobo). Scale with
  ...
  in-flight HF: `docs/content/en/5.dev/06.architecture.md` User interface
...
- Reviews: keep code human-readable, reject exponential/unbounded cost,
  and follow `docs/content/en/5.dev/06.architecture.md`. See
...
| Touch / lift-off / hold | `.cursor/rules/ui-touch.mdc`, `docs/content/en/5.dev/06.architecture.md` (Touch interaction) |
...
| Layers, threads, blackboards, HTTP | `docs/content/en/5.dev/06.architecture.md` |
| UI colour, EFB / NASA HF | `docs/content/en/5.dev/06.architecture.md` (User interface guidelines) |
